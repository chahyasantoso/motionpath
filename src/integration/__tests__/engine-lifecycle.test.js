...
describe('integration: Engine -> Motion -> Track -> compose -> domRenderer', () => {
  it('drives a manual motion all the way to a real DOM write', async () => {
    await engine.loadProject(v4Project); const motion = engine.mountInstance('manual-scrubber'); const needle = motion.getTrack('needle'); expect(needle).toBeTruthy();
    const el = document.createElement('div'); document.body.appendChild(el); const patches = [];
    const unsubscribe = needle.subscribe((raw) => { const patch = needle.compose(raw); patches.push(patch); domRenderer(el, patch); });
    motion.seek(0); expect(patches.at(-1).rotation).toBeCloseTo(-90, 3);
    motion.seek(0.5); expect(patches.at(-1).rotation).toBeCloseTo(0, 3);
    motion.seek(1); expect(patches.at(-1).rotation).toBeCloseTo(90, 3);
    expect(el.getAttribute('style')).toBeTruthy(); unsubscribe();
  });
  it('resolves a template into three tracks and staggers them in SECONDS', async () => {
    await engine.loadProject(v4Project); const motion = engine.mountInstance('time-loop');
    expect(motion.getTrack('card-1')).toBeTruthy(); expect(motion.getTrack('card-2')).toBeTruthy(); expect(motion.getTrack('card-3')).toBeTruthy();
    const card1 = motion.getTrack('card-1'); const patch = card1.compose(card1.getSnapshot()); expect(patch).toHaveProperty('opacity'); expect(patch).toHaveProperty('scale');
    const card3 = motion.getTrack('card-3'); expect(card3.compose(card3.getSnapshot())).toHaveProperty('--card-size'); expect(card1.duration).toBeCloseTo(0.6, 3); expect(motion.getTrack('card-2').duration).toBeCloseTo(0.9, 3);
  });
  it('mounts an image-sequence track and emits a backgroundImage', async () => { await engine.loadProject(v4Project); const motion = engine.mountInstance('sequence'); const strip = motion.getTrack('film-strip'); motion.seek(1); const patch = strip.compose(strip.getSnapshot()); expect(String(patch.backgroundImage)).toContain('003.webp'); });
});
