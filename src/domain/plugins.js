...
export const splitTextPlugin = unsupported('splitText', 'splitText'); export const morphSvgPlugin = unsupported('morphSVG', 'morphSVG'); export const drawSvgPlugin = unsupported('drawSVG', 'drawSVG'); export const scrambleTextPlugin = unsupported('scrambleText', 'scrambleText');
...
export function createPluginRegistry(initialPlugins = ALL_PLUGINS) {
  ...
  const internalKeySet = new Set(); const serializerMap = new Map();
  ...
  const register = (plugin) => {
    ...
    plugins.push(plugin); for (const key of plugin.keys || []) exact.set(key, plugin); if (!(plugin.keys || []).length || plugin.claimsWildcard) predicates.push(plugin); rebuildMetadata(); return plugin;
  };
  const unregister = (pluginOrKey) => { const plugin = typeof pluginOrKey === 'string' ? exact.get(pluginOrKey) : pluginOrKey; if (!plugin || !plugins.includes(plugin)) return false; plugins.splice(plugins.indexOf(plugin), 1); for (const key of plugin.keys || []) if (exact.get(key) === plugin) exact.delete(key); const index = predicates.indexOf(plugin); if (index >= 0) predicates.splice(index, 1); loadPromises.delete(plugin); rebuildMetadata(); return true; };
  const resetLoadPromises = () => { loadPromises.clear(); };
  initialPlugins.forEach(register);
  return { get plugins() { return plugins; }, get internalKeys() { return internalKeySet; }, get serializers() { return serializerMap; }, register, unregister, resetLoadPromises, resolve(key) { return exact.get(key) ?? predicates.find((plugin) => plugin.claimsKey(key)); }, ensureLoaded(plugin) { if (!plugin?.lazy) return Promise.resolve(); if (!loadPromises.has(plugin)) loadPromises.set(plugin, typeof plugin.load === 'function' ? plugin.load() : Promise.resolve()); return loadPromises.get(plugin); } };
}
