...
export function createPluginRegistry(initialPlugins = ALL_PLUGINS) {
  ...
  const rebuildMetadata = () => {
    internalKeySet.clear(); serializerMap.clear();
    for (const plugin of plugins) {
      for (const key of plugin.internalKeys || []) internalKeySet.add(key);
      for (const [key, output] of Object.entries(plugin.outputs || {})) if (typeof output?.serialize === 'function') serializerMap.set(key, output.serialize);
    }
  };
