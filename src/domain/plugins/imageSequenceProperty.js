...
export function warmFrames(frames) {
  ...
  const promise = Promise.all(frames.map((src) => new Promise((resolve) => { const image = new Image(); image.onload = resolve; image.onerror = resolve; image.src = src; }))).then(() => undefined);
...
export function createImageSequencePlugin() {
  return createAnimationPlugin({
    ...
    prepare(trackConfig) { const frames = trackConfig?.keyframes?.imageSequence?.frames; return Array.isArray(frames) ? warmFrames(frames) : undefined; },
    contribute(propKey, stops, elementCfg) {
      ...
      const percentPatch = {};
      stops.forEach((stop) => { const key = toPercentKey(stop.p); percentPatch[key] = { imageSequenceIndex: Number(stop.v) }; if (stop.ease) percentPatch[key].ease = stop.ease; });
