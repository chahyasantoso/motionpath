...
describe("imageSequence plugin", () => {
  ...
  describe("compose()", () => {
    ...
    it("returns empty patch when config is missing", () => {
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 1 }, {}),
      ).toEqual({});
    });

    it("rounds imageSequenceIndex to nearest frame", () => {
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 0 }, elementCfg),
      ).toEqual({
        backgroundImage: "url(a.jpg)",
      });
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 0.4 }, elementCfg),
      ).toEqual({
        backgroundImage: "url(a.jpg)",
      });
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 0.5 }, elementCfg),
      ).toEqual({
        backgroundImage: "url(b.jpg)",
      });
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 1.2 }, elementCfg),
      ).toEqual({
        backgroundImage: "url(b.jpg)",
      });
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 1.6 }, elementCfg),
      ).toEqual({
    ...
    it("clamps index to valid frame range", () => {
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: -10 }, elementCfg),
      ).toEqual({
        backgroundImage: "url(a.jpg)",
      });
      expect(
        imageSequencePlugin.compose({ imageSequenceIndex: 100 }, elementCfg),
      ).toEqual({
        backgroundImage: "url(c.jpg)",
      });
    });
