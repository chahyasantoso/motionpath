...
function sortPlugins(plugins) {
  return plugins
    ...
    .sort(
      (a, b) =>
        (STAGE_ORDER.get(a.plugin.stage) ?? 100) -
          (STAGE_ORDER.get(b.plugin.stage) ?? 100) ||
...
export function buildTrackTween(
  ...
  for (const key of propKeys) {
    ...
    if (!plugin)
      throw new Error(
        `No plugin found for key "${key}" on track "${trackId}".`,
      );
