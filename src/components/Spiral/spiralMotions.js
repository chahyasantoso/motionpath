export function createSpiralContainerScene({ spawnIntervalMs }) {
  return {
    ...
    tracks: [{
      id: 'keepalive',
      duration: 1,
      keyframes: { opacity: { stops: [{ p: 0, v: 0 }, { p: 1, v: 0 }] } },
    }],
  };
}
...
export function createSpiralBallScene({ spiralPathPoints, ballTravelSeconds, ballSize }) {
  return {
    ...
    tracks: [{
      ...
      keyframes: {
        path: { points: spiralPathPoints, stops: [{ p: 0, v: 0 }, { p: 1, v: 1, ease: 'none' }] },
...
export function createSpiralTransitionScene({ ballSize }) {
  return {
    ...
    tracks: [
      { id: 'ball-exit-track', duration: 0.35, keyframes: { scale: { stops: [{ p: 0, v: 1 }, { p: 0.35, v: 1.7 }, { p: 1, v: 0 }] }, opacity: { stops: [{ p: 0, v: 1 }, { p: 0.5, v: 0.9 }, { p: 1, v: 0 }] }, '--ball-size': { stops: [{ p: 0, v: `${ballSize}px` }, { p: 1, v: `${ballSize}px` }] } } },
      { id: 'ball-entrance-track', duration: 0.35, keyframes: { scale: { stops: [{ p: 0, v: 1 }, { p: 0.35, v: 1.7 }, { p: 1, v: 1 }] }, opacity: { stops: [{ p: 0, v: 0 }, { p: 1, v: 1 }] }, '--ball-size': { stops: [{ p: 0, v: `${ballSize}px` }, { p: 1, v: `${ballSize}px` }] } } },
    ],
  };
}

export function createSpiralProject({ spiralPathPoints, ballTravelSeconds, ballSize, spawnIntervalMs }) {
  return {
    schemaVersion: 4,
