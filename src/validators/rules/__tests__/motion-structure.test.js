...
describe("motion-structure rule", () => {
  ...
  it("should error on duplicate templateIds", () => {
    const schema = {
      templates: [{ templateId: "t1" }, { templateId: "t1" }],
    };
    const errors = motionStructureRule(schema);
  ...
  it("rejects motionId as a v3 field and still demands id", () => {
    ...
    expect(
      errors.find((e) => e.path === "motions[0].motionId").message,
  ...
  it("should error if trigger.type is invalid", () => {
    const schema = {
      motions: [{ id: "m1", trigger: { type: 123 }, tracks: [{ id: "tr1" }] }],
    };
    const errors = motionStructureRule(schema);
    expect(errors).toHaveLength(1);
    expect(errors[0].path).toBe("motions[0].trigger.type");
    expect(errors[0].message).toContain(
      "trigger.type is required and must be a string",
    );
  });

  it("should error if tracks is missing or empty", () => {
    const schema1 = {
      motions: [{ id: "m1", trigger: { type: "time" }, tracks: [] }],
    };
    const schema2 = {
      motions: [{ id: "m1", trigger: { type: "time" } }],
    };
  ...
  it("should error on missing id", () => {
    const schema = {
      motions: [{ trigger: { type: "time" }, tracks: [{ id: "tr1" }] }],
    };
    const errors = motionStructureRule(schema);
    expect(errors).toHaveLength(1);
  ...
  it("should error on empty string id", () => {
    const schema = {
      motions: [{ id: "", trigger: { type: "time" }, tracks: [{ id: "tr1" }] }],
    };
    const errors = motionStructureRule(schema);
    expect(errors).toHaveLength(1);
    expect(errors[0].path).toBe("motions[0].id");
