...
describe('element-uniqueness rule', () => {
  it('should pass when track IDs are unique within the same motion', () => {
    const motions = [
      {
  ...
  it('should pass when the same track ID is used across different motions', () => {
    const motions = [
      {
        tracks: [{ id: 'el-1' }]
      },
  ...
  it('should error when a track ID is duplicated within the same motion', () => {
    ...
    expect(errors[0].message).toContain("Duplicate track ID 'el-1' found within the same motion");
  });
});
