export function pathShapeRule(track, motion, context, path) {
  ...
  if (
    ...
    (!anchor ||
      ...
      typeof anchor.xPercent !== "number" ||
      typeof anchor.yPercent !== "number")
  ) {
    errors.push({
      ruleId: "path-shape",
      severity: "error",
      message:
        'path.anchor must be "center", "none", or an object with numeric xPercent and yPercent.',
      path: `${path}.keyframes.path.anchor`,
    });
  }
  if (points.length < 2) {
    errors.push({
    ...
    stops.forEach((stop, index) => {
      ...
      if (
        ...
        errors.push({
          ruleId: "path-shape",
          severity: "error",
          message: `path.stops[${index}].v must satisfy 0 <= v <= 1. Got: ${JSON.stringify(value)}.`,
          path: `${path}.keyframes.path.stops[${index}].v`,
        });
    });
  return errors;
}
