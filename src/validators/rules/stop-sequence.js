  ...
  for (const [key, config] of Object.entries(keyframes)) {
    ...
    stops.forEach((stop, index) => {
      ...
      if (seen.has(stop.p)) {
        errors.push({
          ...
          severity: "error",
          message: `Duplicate stop position ${stop.p} for property "${key}".`,
          path: `${propPath}[${index}].p`,
        });
      } else {
      ...
      if (stop.p < previous) {
        errors.push({
          ruleId: "stop-sequence",
          severity: "error",
          message: `Stop positions for property "${key}" must be monotonic.`,
          path: `${propPath}[${index}].p`,
        });
      }
      previous = stop.p;
    ...
    if (!stops.some((stop) => stop && stop.p === 0)) {
      errors.push({
        ruleId: "stop-sequence",
        severity: "warning",
        message: `Property "${key}" has no p: 0 stop; its first frame may be undefined.`,
        path: propPath,
      });
    }
    if (!stops.some((stop) => stop && stop.p === 1)) {
      errors.push({
        ruleId: "stop-sequence",
        severity: "warning",
        message: `Property "${key}" has no p: 1 stop.`,
        path: propPath,
      });
    }
  }
