...
const base = (key) => ({ schemaVersion: 4, motions: [{ id: 'm', trigger: { type: 'manual' }, tracks: [{ id: 't', keyframes: { [key]: { stops: [{ p: 0, v: 0 }, { p: 1, v: 1 }] } } }] }] });
...
describe('Engine registry isolation', () => {
  it('does not share custom plugins between Engine instances', async () => {
    ...
    await expect(b.loadProject(base('onlyA'))).rejects.toThrow(/No plugin found/);
  });
