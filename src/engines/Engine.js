...
export class Engine {
  ...
  #plugins;
  #triggers;
  constructor({ eventBus = new EventBus(), plugins = createPluginRegistry(), triggerDelegates = createTriggerDelegateRegistry() } = {}) {
  ...
  #trackOptions() { return { eventBus: this.#eventBus, resolvePluginForKey: (key) => this.#plugins.resolve(key) }; }
  ...
  #mountMotion(config, delegate) {
    ...
    const entries = (config.tracks || []).map((trackConfig, index) => {
      const track = createTrack(trackConfig, this.#v4Project.templates, this.#trackOptions());
  ...
  mountInstance(id) {
    ...
    if (config) {
      ...
      if (!factory) throw new Error(`Unknown trigger type "${config.trigger?.type}" on motion "${id}".`);
  ...
  unmount(object) {
    ...
    if (handle !== undefined) { this.#handles.delete(object); this.#instances.delete(handle); }
    object.destroy?.();
  ...
  getTrack(id) {
    ...
    for (const object of this.#instances.values()) if (object && typeof object.progress === "function" && object.id === id) return object;
    return null;
  }
