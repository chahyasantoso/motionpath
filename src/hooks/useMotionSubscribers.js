...
export default function useMotionSubscribers(sources, ref, mergeFn) {
  ...
  useEffect(() => {
    ...
    const latestPatches = stableSources.map(() => ({})); let dirty = false;
    const render = () => { if (!ref.current || !dirty) return; dirty = false; const merge = mergeFnRef.current ?? ((patches) => Object.assign({}, ...patches)); domRenderer(ref.current, merge(latestPatches)); };
