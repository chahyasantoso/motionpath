...
function sourcesSignature(sources) {
  return sources.map((s) => `${s.instance?.id ?? ''}:${s.track?.id ?? s.trackId ?? ''}`).join('|');
}
...
function subscribeToSource(source, getTransformFn, getAnchor, onPatch) {
  ...
  if (instance?.subscribe && trackId) {
    ...
    return instance.subscribe(trackId, (raw) => {
      const transformFn = getTransformFn();
      onPatch(applyAnchor(typeof transformFn === 'function' ? transformFn(raw, compose) : compose(raw), getAnchor()));
    });
  }
  return () => {};
}
...
export default function useMotionSubscribers(sources, ref, mergeFn) {
  ...
  useEffect(() => {
    ...
    const useTicker = Boolean(ticker && typeof ticker.add === 'function' && typeof ticker.remove === 'function');
    if (useTicker) ticker.add(render);
    const unsubscribes = stableSources.map((source, index) => subscribeToSource(source, () => transformFnsRef.current[index], () => anchorsRef.current[index], (patch) => {
      // Preserve the source slot even when a callback supplies a partial patch.
      // This keeps disjoint sources composable across successive ticks and makes
      // the immediate fallback behave exactly like the pre-coalescing hook.
      latestPatches[index] = { ...latestPatches[index], ...(patch || {}) };
      dirty = true;
      if (!useTicker) render();
    }));
    return () => { unsubscribes.forEach((unsubscribe) => unsubscribe()); if (useTicker) ticker.remove(render); };
  }, [ref, stableSources]);
}
