...
export class Track {
  ...
  #host = null; #parent = null; #children = new Map(); #subscribers = new Set(); #currentOffset = 0; #staggerOffset = 0; #layoutDelegate; #observed = new Map();
  ...
  progress(p) { if (p === undefined) return this.#interpolationTimeline.progress(); this.#interpolationTimeline.progress(clamp01(p)); this.#notify(); }
  getSnapshot() { const { _gsap, ...rest } = this.#proxyState; return { ...rest, progress: this.#interpolationTimeline.progress() }; }
  compose(rawData, ctx) {
    ctx = ctx ?? new Map(); const cached = ctx.get(this);
    if (cached === COMPOSING) return composePatch(this.#plugins, rawData ?? this.getSnapshot(), this.#resolvedTrack, `track "${this.#id}"`);
    if (cached !== undefined) return cached;
    ctx.set(this, COMPOSING); let source = rawData ?? this.getSnapshot();
    for (const [observedSource, { mapFn, role }] of this.#observed) if (role === 'input' && mapFn) { const contribution = mapFn(observedSource.compose(undefined, ctx)); if (contribution) source = { ...source, ...contribution }; }
    let patch = composePatch(this.#plugins, source, this.#resolvedTrack, `track "${this.#id}"`);
    for (const [observedSource, { mapFn, role }] of this.#observed) if (role === 'output' && mapFn) { const observedPatch = mapFn(observedSource.compose(undefined, ctx)); if (observedPatch) patch = mergePatches(patch, observedPatch); }
    ctx.set(this, patch); return patch;
  }
  setObserved(track, mapFn, opts = {}) { if (!track) { this.#observed.clear(); return; } this.#observed.set(track, { mapFn: mapFn ?? null, role: opts.role ?? 'output' }); }
  ...
  _mount(host) { if (this.#host) throw new Error(`Track "${this.#id}" already mounted to "${this.#host.id ?? 'another motion'}"`); this.#host = host; }
