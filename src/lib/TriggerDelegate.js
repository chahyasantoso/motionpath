...
export class ScrollTriggerDelegate {
  ...
  buildTimelineVars() {
    ...
    const scrollTrigger = { start: c.start, end: c.end, scrub: c.scrub, pin: c.pin === true ? trigger : c.pin, pinSpacing: c.pinSpacing, toggleActions: c.toggleActions };
  ...
  destroy() { this.#timeline?.scrollTrigger?.kill(true); this.#timeline?.kill(); }
}
export class TimeTriggerDelegate {
  #config; #controls; #timeline;
  constructor(config = {}) { this.#config = config; }
  build() { const c = this.#config; this.#timeline = gsap.timeline({ repeat: c.repeat ?? 0, yoyo: !!c.yoyo, repeatDelay: c.repeatDelay ?? 0, delay: typeof c.delay === "number" ? c.delay : 0, paused: !(c.autoplay ?? true) }); this.#controls = new AutonomousTimelineControls(this.#timeline); return this.#timeline; }
  play() { this.#controls?.play(); }
  pause() { this.#controls?.pause(); }
  seek(p) { this.#controls?.seek(p); }
  reverse() { this.#controls?.reverse(); }
...
export class ManualTriggerDelegate {
  #timeline; #controls;
  build() { this.#timeline = gsap.timeline({ paused: true }); this.#controls = new AutonomousTimelineControls(this.#timeline); return this.#timeline; }
  seek(p) { if (!this.#timeline) return 0; if (p === undefined) return this.#timeline.progress(); this.#controls.seek(p); }
...
export function createTriggerDelegateRegistry(initial = [["scroll", (c) => new ScrollTriggerDelegate(c)], ["time", (c) => new TimeTriggerDelegate(c)], ["manual", (c) => new ManualTriggerDelegate(c)]]) {
  const registry = new Map(initial);
  return { get: (type) => registry.get(type), has: (type) => registry.has(type), set: (type, factory) => registry.set(type, factory), delete: (type) => registry.delete(type), entries: () => registry.entries() };
}
