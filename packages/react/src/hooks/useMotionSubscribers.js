...
import { domRenderer, clearRendererTarget } from "../renderers/domRenderer.js";
import { applyAnchor } from "@motionpath/core/helpers.js";
...
function subscribeToSource(source, getTransformFn, getAnchor, onPatch) {
  ...
  if (targetTrack?.subscribe) return targetTrack.subscribe((raw) => {
  ...
  if (instance?.subscribe && trackId) return instance.subscribe(trackId, (raw) => {
    const transformFn = getTransformFn();
...
export default function useMotionSubscribers(sources, ref, mergeFn) {
  ...
  useEffect(() => {
    ...
    const unsubscribes = stableSources.map((source, index) => subscribeToSource(source, () => transformFnsRef.current[index], () => anchorsRef.current[index], (patch) => { latestPatches[index] = patch || {}; dirty = true; if (!useTicker) render(); }));
    return () => {
