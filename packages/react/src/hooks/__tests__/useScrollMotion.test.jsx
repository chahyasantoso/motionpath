...
describe("useScrollMotion", () => {
  ...
  afterEach(() => cleanup());
  it("mounts shared trigger motions", () => {
    const { getByTestId } = render(<SharedTriggerHarness />);
    expect(engine.mountWithDelegate).toHaveBeenCalledTimes(2);
    expect(engine.mountWithDelegate.mock.calls[0][0]).toBe("scrub-scene");
    expect(engine.mountWithDelegate.mock.calls[1][0]).toBe("observer-scene");
  });
  it("allocates own refs", () => {
    render(<OwnRefsHarness />);
    expect(engine.mountWithDelegate).toHaveBeenCalledTimes(1);
  });
  it("defers null schema", () => {
    render(<NullSchemaHarness />);
    expect(engine.mountWithDelegate).not.toHaveBeenCalled();
  });
  it("unmounts through engine", () => {
