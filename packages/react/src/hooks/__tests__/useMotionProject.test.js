...
vi.mock("../../../../../packages/core/src/engines/Engine.js", () => ({ engine: { loadProject: vi.fn(), destroy: vi.fn() } }));
...
describe("useMotionProject", () => {
  ...
  it("reloads on a new reference with the same id", () => { engine.loadProject.mockResolvedValue(); const project = { schemaVersion: 2, projectId: "p1", motions: [] }; const { rerender } = renderHook(({ p }) => useMotionProject(p), { initialProps: { p: project } }); rerender({ p: { ...project } }); expect(engine.loadProject).toHaveBeenCalledTimes(2); });
});
