import {
  parseV4Project,
  FREE_TRACK_NAMESPACE,
} from "../lib/schema/parseV4Project.js";
import {
...
function resolveObservationOwnership({ projectRuntime, observationOwnership }) {
  ...
  if (injected !== undefined) {
    if (
      observationOwnership !== undefined &&
      observationOwnership !== injected
    ) {
      throw new TypeError(
        `Engine observationOwnership '${observationOwnership}' conflicts with the injected ProjectRuntime ownership '${injected}'.`,
      );
    }
    return injected;
  }
  ...
  if (!OBSERVATION_OWNERSHIP_MODES.includes(requested)) {
    throw new TypeError(
      "Engine observationOwnership must be 'compatibility' or 'scoped'.",
    );
  }
  return requested;
}
...
export class Engine {
  ...
  #lastValidation = [];
  #dependencies;
  #publisherRendering = false;
  #clock = null;
  #ownsClock = false;
  #observationOwnership;
  constructor(options = {}) {
    this.#dependencies =
      options.dependencies || createRuntimeDependencies(options);
  ...
  get publisherRendering() {
    return this.#publisherRendering;
  }
  set publisherRendering(value) {
    this.#publisherRendering = value === true;
  }
  get clock() {
    return this.#clock;
  }
  async loadProject(schema, options = {}) {
    ...
    try {
      for (const [id, config] of candidate.qualifiedTrackConfigs ?? [])
        this.#projectRuntime.registerCandidate(staged, id, config, {
          kind: "track-config",
        });
  ...
  #register(object, kind) {
    ...
    this.#projectRuntime.registerInstance(handle, object, { kind });
    return handle;
  }
  ...
  #trackOptions(extra = {}) {
    return {
      ...
      observationAdapter: this.#projectRuntime.standaloneObservationAdapter,
      ...extra,
    };
  }
  #resolveClock() {
    if (!this.#clock || this.#clock.isDisposed) {
      this.#clock = createTickClock(gsapTickerClock);
      this.#ownsClock = true;
    }
    return this.#clock;
  }
  #mountMotion(config, delegate) {
    const graph = normalizeObservationGraph(config);
    ...
    try {
      ...
      const entries = (config.tracks || []).map((trackConfig, index) => {
        ...
        tracks.push(track);
        motion.mount(track, index * stagger);
        return { track, config: trackConfig };
      });
      const trackMap = new Map(entries.map(({ track }) => [track.id, track]));
      const initialEdges = entries.flatMap(({ track, config: trackConfig }) =>
        (trackConfig.observes || []).map((edge) => ({
          source: edge.source,
          target: track.id,
          ...
          mapFn:
            edge.role === "input"
              ? (patch) => ({ [edge.target]: patch || {} })
              : (patch) => patch,
        })),
      );
      if (this.#publisherRendering) {
        runtime = new GraphRuntime({ graph, tracks: trackMap, initialEdges });
        motion.setGraphRuntime(runtime);
      } else {
        publisher = new GraphPublisher({
          graph,
          tracks: trackMap,
          publish: () => {},
        });
        binding = new GraphBinding({
          graph,
          tracks: trackMap,
          publisher,
          initialEdges,
  ...
  mountInstance(id) {
    ...
    if (config) {
      const factory = this.#dependencies.triggerDelegates.get(
        config.trigger?.type,
      );
      if (!factory)
        throw new Error(
    ...
    const qualified =
      this.#projectRuntime.getProjectLookup(id) ??
      this.#v4Project.getQualifiedTrackConfig?.(id);
    if (qualified) {
      ...
      if (namespace !== FREE_TRACK_NAMESPACE) runtime.motionId = namespace;
      runtime.trackId = qualified.id;
      runtime.qualifiedId = id;
      this.#register(
        runtime,
        namespace === FREE_TRACK_NAMESPACE ? "free-track" : "qualified-track",
      );
      return runtime;
    }
  ...
  createMotionHost({ id, staggerTransition = {}, autoplay = true } = {}) {
    if (!this.#v4Project)
      throw new Error("createMotionHost: project not loaded.");
    if (typeof id !== "string" || id.length === 0)
      throw new TypeError("createMotionHost: id must be a non-empty string.");
    const motion = new Motion({
      id: `host-${id}`,
      triggerDelegate: new TimeTriggerDelegate({ autoplay }),
      staggerTransition,
    });
    const track = createTrack(
    ...
    motion.mount(track, 0);
    motion.init();
    this.#register(motion, "motion-host");
    return { motion, track };
  }
  adopt(object) {
  ...
  unmount(object) {
    ...
    this.#projectRuntime.unregisterInstance(handle, { destroy: true });
    return true;
  }
  isOwned(object) {
  ...
  getTrackConfig(id) {
    try {
      return (
        this.#projectRuntime.getProjectLookup(id) ??
        this.#v4Project?.getTrackConfig(id) ??
        null
      );
    } catch (error) {
      if (/^Ambiguous track id/.test(error?.message ?? "")) return null;
      throw error;
    }
  }
  get templates() {
  ...
  destroy() {
    if (this.#projectRuntime && !this.#projectRuntime.isDisposed)
      this.#projectRuntime.dispose();
    this.#projectRuntime = new ProjectRuntime({
      observationOwnership: this.#observationOwnership,
    });
