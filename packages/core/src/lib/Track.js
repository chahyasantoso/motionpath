...
import { createDestroyEvent, installLegacyObservationFacade } from "../usecases/LegacyObservationFacade.js";
 ...
 *
 * Legacy observation names intentionally live in the external facade, not here.
 */
export class Track {
  ...
  #staggerOffset = 0; #layoutDelegate; #standaloneObservationAdapter; #observationController = null;
  ...
  constructor({ id, mode = "standalone", interpolationTimeline, proxyState, plugins, resolvedTrack, layoutDelegate, eventBus = defaultEventBus, observationAdapter = null }) {
    ...
    this.#interpolationTimeline = interpolationTimeline; this.#proxyState = proxyState; this.#plugins = plugins;
    this.#resolvedTrack = resolvedTrack; this.#layoutDelegate = layoutDelegate ?? defaultGaplessLayoutDelegate;
    this.#eventBus = eventBus; this.#standaloneObservationAdapter = this.#mode === "standalone"
      ? (observationAdapter ?? defaultProjectRuntime.standaloneObservationAdapter) : null;
    this.#standaloneObservationAdapter?.register(this); installLegacyObservationFacade(this);
  }
  get id() { return this.#id; } get mode() { return this.#mode; } get currentOffset() { return this.#currentOffset; }
  get parent() { return this.#parent; } get duration() { return this.#interpolationTimeline?.duration() ?? 0; }
  ...
  _unmount() { this.#host = null; } get isMounted() { return this.#host !== null; } get childCount() { return this.#children.size; } getChild(id) { return this.#children.get(id) ?? null; }
  ...
  play() { this.#groupHost?.timeline.play(); } pause() { this.#groupHost?.timeline.pause(); } seek(progress) { if (!this.#groupHost) return this.progress(progress); if (progress === undefined) return this.#groupHost.timeline.progress(); this.#groupHost.timeline.progress(clamp01(progress)); } reverse() { this.#groupHost?.timeline.reverse(); }
  /** Owner cleanup runs before the destroy snapshot is delivered. */
  destroy() { if (this.#destroyed || this.#destroying) return; this.#destroying = true; const ids = this.#owner()?.getObserverIds?.(this) ?? []; for (const callback of [...this.#destroySubscribers]) callback(createDestroyEvent(this.#id, ids)); this.#owner()?.clearObserved?.(this); this.#owner()?.removeSourceEdges?.(this); this.#destroyed = true; this.#standaloneObservationAdapter?.unregister(this); for (const child of this.#children.values()) child.destroy(); this.#children.clear(); this.#parent = null; this.#host = null; this.#subscribers.clear(); this.#emit({ type: "destroyed", track: this, ids }); this.#destroySubscribers.clear(); this.#lifecycleSubscribers.clear(); if (this.#groupHost) { this.#groupHost.group.destroy(); this.#groupHost.timeline.kill(); this.#groupHost = null; } try { this.#interpolationTimeline?.kill(); } catch (error) { logger.warn(`track "${this.#id}", failed to kill timeline during destroy()`, error); } }
