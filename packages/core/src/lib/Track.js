...
export class Track {
  ...
  #staggerOffset = 0; #layoutDelegate; #standaloneObservationAdapter;
  #observationController = null; #groupHost = null; #destroyed = false; #destroying = false;
  ...
  constructor({ id, mode = "standalone", interpolationTimeline, proxyState, plugins, resolvedTrack, layoutDelegate, eventBus = defaultEventBus, observationAdapter = null }) {
    this.#id = id; this.#mode = mode === "authored-graph" ? "authored-graph" : "standalone";
    this.#interpolationTimeline = interpolationTimeline; this.#proxyState = proxyState;
    this.#plugins = plugins; this.#resolvedTrack = resolvedTrack;
    this.#layoutDelegate = layoutDelegate ?? defaultGaplessLayoutDelegate; this.#eventBus = eventBus;
  ...
  get id() { return this.#id; } get mode() { return this.#mode; }
  get currentOffset() { return this.#currentOffset; } get parent() { return this.#parent; }
  ...
  progress(progress) {
    ...
    if (this.#destroyed) return; this.#interpolationTimeline?.progress(clamp01(progress));
    this.#notify(); this.#invalidate("progress");
  }

  getSnapshot() {
    this.#assertAlive(); const { _gsap, ...rest } = this.#proxyState || {};
    return { ...rest, progress: this.#interpolationTimeline?.progress() ?? 0 };
  }

  composeLocal(raw) {
    this.#assertAlive(); return composePatch(this.#plugins, raw ?? this.getSnapshot(), this.#resolvedTrack, `track "${this.#id}"`);
  }
  ...
  onLifecycle(callback) {
    ...
    this.#lifecycleSubscribers.add(callback); return () => this.#lifecycleSubscribers.delete(callback);
  }

  onSourceDestroyed(callback) {
    if (typeof callback !== "function") throw new TypeError("Track destroy callback must be a function.");
    this.#destroySubscribers.add(callback); return () => this.#destroySubscribers.delete(callback);
  }
  ...
  /** Move a directly-created Track into the caller's explicit compatibility scope. */
  _adoptObservationOwner(owner) {
    if (this.#mode !== "standalone" || !owner || owner === this.#standaloneObservationAdapter) return;
  ...
  _mount(host) {
    ...
    if (this.#host) throw new Error(`Track "${this.#id}" already mounted`); this.#host = host;
  }
  _unmount() { this.#host = null; } get isMounted() { return this.#host !== null; }
  get childCount() { return this.#children.size; } getChild(id) { return this.#children.get(id) ?? null; }
  ...
  addChild(child, opts = {}) {
    ...
    child.#parent = this; const stagger = opts.stagger ?? 0; child.#staggerOffset = stagger;
    child.#currentOffset = this.#layoutDelegate.computeSpawnOffset([...this.#children.values()], { stagger });
    this.#children.set(child.id, child); if (this.#host) this.#host._mountChild(child, child.#currentOffset);
    this.#eventBus.emit("child:spawned", { id: child.id, parentId: this.#id }); this.#invalidate("children");
  }

  removeChild(id) {
    const child = this.#children.get(id); if (!child) return; const siblings = [...this.#children.values()];
    this.#children.delete(id); child.#parent = null; child.#owner()?.removeSourceEdges?.(child); child.#owner()?.clearObserved?.(child);
    if (this.#host) this.#host._unmountChild(child);
    for (const target of this.#layoutDelegate.computeReflow(siblings, child, {})) { target.child.#currentOffset = target.offset; if (this.#host) this.#host._reflowChild(target.child, target.offset); }
    child.#emit({ type: "detached", track: child, parent: this }); this.#eventBus.emit("child:removing", { id: child.id, parentId: this.#id });
  }
  ...
  play() { this.#groupHost?.timeline.play(); } pause() { this.#groupHost?.timeline.pause(); }
  ...
  /** Owner cleanup runs before the destroy snapshot is delivered. */
  destroy() {
    if (this.#destroyed || this.#destroying) return; this.#destroying = true;
    ...
    this.#owner()?.clearObserved?.(this); this.#owner()?.removeSourceEdges?.(this); this.#destroyed = true;
    this.#standaloneObservationAdapter?.unregister(this); for (const child of this.#children.values()) child.destroy();
    this.#children.clear(); this.#parent = null; this.#host = null; this.#subscribers.clear(); this.#emit({ type: "destroyed", track: this });
    ...
    try { this.#interpolationTimeline?.kill(); } catch (error) { logger.warn(`track "${this.#id}", failed to kill timeline during destroy()`, error); }
  }
