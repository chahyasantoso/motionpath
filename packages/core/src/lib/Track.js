...
export class Track {
  ...
  destroy() { if (this.#destroyed || this.#destroying) return; this.#destroying = true; const observerIds = this.#owner()?.getObserverIds?.(this) ?? []; for (const callback of [...this.#destroySubscribers]) callback({ id: this.#id, observerIds }); this.#owner()?.clearObserved?.(this); this.#owner()?.removeSourceEdges?.(this); this.#destroyed = true; this.#standaloneObservationAdapter?.unregister(this); for (const child of this.#children.values()) child.destroy(); this.#children.clear(); this.#parent = null; this.#host = null; this.#subscribers.clear(); this.#emit({ type: "destroyed", track: this, observerIds }); this.#destroySubscribers.clear(); this.#lifecycleSubscribers.clear(); if (this.#groupHost) { this.#groupHost.group.destroy(); this.#groupHost.timeline.kill(); this.#groupHost = null; } try { this.#interpolationTimeline?.kill(); } catch (error) { logger.warn(`track "${this.#id}", failed to kill timeline during destroy()`, error); } }
