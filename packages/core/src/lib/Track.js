...
export class Track {
  ...
  #assertAlive() {
    if (this.#destroyed) {
      throw new Error(`Track "${this.#id}" is destroyed.`);
    }
  }
  ...
  _mount(host) {
    ...
    if (this.#host) {
      throw new Error(`Track "${this.#id}" already mounted`);
    }
    this.#host = host;
  }
  ...
  addChild(child, opts = {}) {
    ...
    if (this.#host) {
      this.#host._mountChild(child, child.#currentOffset);
    }
    this.#eventBus.emit("child:spawned", {
      id: child.id,
      parentId: this.#id,
    });
  ...
  removeChild(id) {
    ...
    this.#eventBus.emit("child:removing", {
      id: child.id,
      parentId: this.#id,
    });
  }
