 ...
 * in the injected owner, never in this leaf object.
 */
export class Track {
  #id;
  #mode;
  ...
  /** Snapshot is raw playhead state; plugins run only in compose. */
  getSnapshot() {
    this.#assertAlive();
  ...
  removeChild(id) {
    ...
    child.#parent = null;
    child.#owner()?.removeSourceEdges?.(child);
    ...
    child.#emit({ type: "detached", track: child, parent: this });
  ...
  destroy() {
    ...
    const destroyEvent = createDestroyEvent(this.#id, observerIds);
    for (const callback of [...this.#destroySubscribers]) callback(destroyEvent);
    ...
    this.#emit({ type: "destroyed", track: this, observerIds: [...observerIds] });
