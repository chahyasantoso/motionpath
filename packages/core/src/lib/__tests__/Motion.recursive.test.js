...
describe("recursive Motion scheduling", () => {
  it("schedules a depth-three Motion tree with parent-relative offsets", () => {
    ...
    const child = motion("child");
    const grandchild = motion("grandchild");
    root.init();
    child.init();
    grandchild.init();
    child.mount(track("child-track"));
    grandchild.mount(track("grandchild-track"));
    child.mount(grandchild, 0.25);
    root.mount(child, 0.5);
    expect(child.isMounted).toBe(true);
    expect(grandchild.isMounted).toBe(true);
    ...
    root.seek(2 / 7);
    expect(child.progress()).toBeCloseTo(0, 5);
    root.seek(9 / 14);
