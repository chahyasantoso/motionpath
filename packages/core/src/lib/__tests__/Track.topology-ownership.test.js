...
describe("P2-04 Track topology ownership", () => {
  ...
  it("keeps the Motion-facing composite API available", async () => {
    ...
    expect(Motion.prototype.mountChild).toBeTypeOf("function");
    expect(Motion.prototype.unmountChild).toBeTypeOf("function");
    expect(Motion.prototype.reflowChild).toBeTypeOf("function");
  });
});
