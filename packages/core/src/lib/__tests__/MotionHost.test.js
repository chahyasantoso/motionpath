...
function project() {
  return {
    ...
    tracks: [{ id: "host-template-track", duration: 1, keyframes: {} }],
  };
}
...
describe("Engine.createMotionHost", () => {
  ...
  it("creates a Motion-owned dynamic host", async () => {
    ...
    const { motion, track: host } = engine.createMotionHost({
      id: "spiral-parent",
      autoplay: true,
    });
    ...
    expect(engine.isOwned(motion)).toBe(true);
    motion.pause();
    motion.seek(0);
    motion.play();
  ...
  it("rejects an empty host id", async () => {
    engine = new Engine();
    await engine.loadProject(project());
    expect(() => engine.createMotionHost({ id: "" })).toThrow(
