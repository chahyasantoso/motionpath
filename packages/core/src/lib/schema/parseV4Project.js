...
function assertNamespaceSafeMotionId(id) { if (typeof id === "string" && id.includes("/")) throw new Error(`Motion id "${id}" must not contain "/": that separator is reserved for qualified ids.`); if (id === FREE_TRACK_NAMESPACE) throw new Error(`Motion id "${FREE_TRACK_NAMESPACE}" is reserved for the free-track namespace.`); }
function assertNamespaceSafeTrackId(id) { if (typeof id === "string" && id.includes("/")) throw new Error(`Track id "${id}" must not contain "/": that separator is reserved for qualified ids.`); }
...
export async function parseV4Project(schema = {}, deps = {}) {
  ...
  const register = (namespace, track) => { const trackId = track?.id; assertNamespaceSafeTrackId(trackId); const qualifiedId = qualify(namespace, trackId); if (qualifiedTrackConfigsMap.has(qualifiedId)) throw new Error(`Duplicate qualified track id "${qualifiedId}".`); qualifiedTrackConfigsMap.set(qualifiedId, track); const owners = bareTrackOwners.get(trackId); if (owners) owners.push(qualifiedId); else bareTrackOwners.set(trackId, [qualifiedId]); if (!trackConfigsMap.has(trackId)) trackConfigsMap.set(trackId, track); };
  for (const motion of project.motions) { const type = motion.trigger?.type; if (!type) throw new Error(`Motion "${motion.id}" is missing trigger.type.`); if (!delegates.get(type)) throw new Error(`Unknown trigger type "${type}" on motion "${motion.id}".`); assertNamespaceSafeMotionId(motion.id); motionConfigsMap.set(motion.id, motion); for (const track of motion.tracks || []) { collect(track); register(motion.id, track); } }
