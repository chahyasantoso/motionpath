...
export class TrackGroup {
  ...
  composeGraph() { const patches = new Map(); for (const id of this.#graphOrder) { const track = this.#tracks.get(id); if (track) patches.set(id, track.compose(undefined, patches)); } return patches; }
...
export class Motion {
  id; #triggerDelegate; #group; #active = false; #initialTracks = []; #masterTimeline; #staggerTransition; #graphOrder; #destroyed = false;
