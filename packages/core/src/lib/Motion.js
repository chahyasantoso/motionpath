...
export class TrackGroup {
  ...
  constructor(masterTimeline, staggerTransition = {}, graphOrder = []) {
    this.#masterTimeline = masterTimeline;
    this.#staggerTransition = staggerTransition;
    this.#graphOrder = [...graphOrder];
  }
  mount(track, position) {
    if (this.#tracks.has(track.id)) throw new Error(`TrackGroup already contains track "${track.id}".`);
    track._mount(this);
...
export class Motion {
  ...
  mount(track, position) {
    ...
    if (this.#initialTracks.some((entry) => entry.track.id === track.id)) throw new Error(`Motion "${this.id}" already contains track "${track.id}".`);
