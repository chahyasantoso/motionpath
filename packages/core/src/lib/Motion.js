...
export class TrackGroup {
  ...
  mount(track, position) { track._mount(this); const tween = gsap.to(track, { progress: 1, ease: "none", duration: track.duration || 0, paused: false }); this.#proxies.set(track.id, tween); this.#tracks.set(track.id, track); this.#masterTimeline.add(tween, position); this.#masterTimeline.render(this.#masterTimeline.time(), true, true); }
  ...
  _mountChild(child, spawnOffset) { this.mount(child, spawnOffset); } _unmountChild(child) { this.unmount(child); }
...
export class Motion {
  id; #triggerDelegate; #group; #active = false; #initialTracks = []; #masterTimeline; #staggerTransition; #destroyed = false;
  ...
  init() { if (this.#active) this.destroy(); this.#destroyed = false; this.#active = true; this.#masterTimeline = this.#triggerDelegate.build(); this.#group = new TrackGroup(this.#masterTimeline, this.#staggerTransition); for (const { track, position } of this.#initialTracks) this.#group.mount(track, position); }
  mount(track, position) { if (this.#destroyed) throw new Error(`Motion "${this.id}" is destroyed.`); if (!this.#initialTracks.some((t) => t.track.id === track.id)) this.#initialTracks.push({ track, position }); if (this.#active) this.#group.mount(track, position); }
  ...
  destroy() { if (this.#destroyed) return; this.#destroyed = true; if (this.#group) { this.#group.destroy(); this.#group = null; } for (const { track } of this.#initialTracks) track.destroy?.(); this.#initialTracks = []; this.#triggerDelegate.destroy(); this.#masterTimeline = null; this.#active = false; }
}
