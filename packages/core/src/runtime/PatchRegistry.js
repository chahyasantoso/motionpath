...
function withoutNode(sourceRevisions, nodeId) {
  return Object.fromEntries(
    Object.entries(sourceRevisions ?? {}).filter(([id]) => id !== nodeId),
  );
}
...
function freezePatch(patch) {
  return Object.freeze({
    ...patch,
    values: toImmutablePatchValues(patch.values),
...
export class PatchRegistry {
  ...
  endBatch() {
    if (this.#batchDepth === 0)
      throw new Error("PatchRegistry batch is not open.");
    this.#batchDepth -= 1;
  ...
  publish(
    nodeId,
    values,
    { sourceProgress = 0, sourceRevisions = {}, status = "ready" } = {},
  ) {
    if (typeof nodeId !== "string" || nodeId.length === 0)
      throw new TypeError("PatchRegistry nodeId must be a non-empty string.");
    if (!["ready", "blocked", "error"].includes(status))
      throw new TypeError(`Unknown patch status '${status}'.`);
    const previous = this.#patches.get(nodeId);
    const externalSourceRevisions = withoutNode(sourceRevisions, nodeId);
    if (
      previous &&
      sameValue(previous.values, values) &&
      previous.sourceProgress === sourceProgress &&
      sameValue(
        withoutNode(previous.sourceRevisions, nodeId),
    ...
    const patch = freezePatch({
      ...
      values,
      sourceProgress,
      sourceRevisions: { ...externalSourceRevisions, [nodeId]: revision },
      status,
    });
    this.#revisions.set(nodeId, revision);
    this.#patches.set(nodeId, patch);
    if (this.#batchDepth) this.#pending.add(nodeId);
    else {
  ...
  get(nodeId) {
    return this.#patches.get(nodeId) ?? null;
  }
  snapshot() {
  ...
  subscribe(nodeId, callback) {
    if (typeof callback !== "function")
      throw new TypeError("PatchRegistry subscriber must be a function.");
    const subscribers = this.#subscribers.get(nodeId) ?? new Set();
    subscribers.add(callback);
    this.#subscribers.set(nodeId, subscribers);
    return () => {
      subscribers.delete(callback);
      if (subscribers.size === 0) this.#subscribers.delete(nodeId);
    };
  }
  clear() {
    this.#patches.clear();
  ...
  #notifyNode(nodeId) {
    for (const callback of [...(this.#subscribers.get(nodeId) ?? [])])
      callback(this.#patches.get(nodeId));
  }
  #notifyGlobal() {
