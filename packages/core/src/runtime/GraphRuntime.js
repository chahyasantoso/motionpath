...
export class GraphRuntime {
  ...
  constructor({
    ...
    if (!graph)
      throw new TypeError("GraphRuntime requires a normalized graph.");
    this.#patches = patches;
    this.enabled = enabled !== false;
    const publish = (nodeId, values) => {
      const track = this.#binding?.getTrack(nodeId);
      return this.#patches.publish(nodeId, values, {
        sourceProgress: track?.progress?.(),
        sourceRevisions: Object.fromEntries(
          [...this.#patches.snapshot()].map(([id, patch]) => [
            id,
            patch.revision,
          ]),
        ),
  ...
  get graph() {
    return this.#binding?.graph ?? null;
  }
  get binding() {
    return this.#binding;
  }
  ...
  get isRunning() {
    return this.#clockUnsubscribe !== null;
  }
  register(track, observes = []) {
    this.#assertAlive();
    this.#binding.addTrack(track, { observes });
    return track;
  }
  unregister(id, options) {
    this.#assertAlive();
    this.#binding.removeTrack(id, options);
  }
  replaceEdges(oldEdge, newEdge) {
    this.#assertAlive();
    this.#binding.replaceEdge(oldEdge, newEdge);
  }
  addEdge(edge) {
    this.#assertAlive();
    this.#binding.addEdge(edge);
  }
  getPatch(nodeId) {
  ...
  subscribe(nodeId, callback) {
    this.#assertAlive();
    return this.#patches.subscribe(nodeId, callback);
  }
  compose(nodeId, rawData) {
    this.#assertAlive();
    const track = this.#binding.getTrack(nodeId);
    if (!track) throw new Error(`Unknown runtime node '${nodeId}'.`);
    return track.compose(rawData);
  }
  flush() {
    if (this.#disposed || !this.enabled || this.#flushInProgress) return 0;
    this.#flushInProgress = true;
    this.#patches.beginBatch();
    try {
      return this.#publisher.flush();
    } finally {
      this.#patches.endBatch();
  ...
  start(clock) {
    ...
    if (!clock || typeof clock.subscribe !== "function")
      throw new TypeError("GraphRuntime.start requires a clock.");
    if (this.#clockUnsubscribe) this.#clockUnsubscribe();
    this.#publisher.markAllDirty();
    this.#clockUnsubscribe = clock.subscribe(({ tick } = {}) => {
      this.lastTick = tick ?? (this.lastTick ?? 0) + 1;
      try {
        this.flush();
      } catch (error) {
...
export function createGraphRuntime(motion, options = {}) {
  ...
  if (!graph.valid)
    throw new Error(
      `Cannot create runtime from invalid graph: ${graph.errors.map((error) => error.message).join("; ")}`,
    );
