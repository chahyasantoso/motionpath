...
describe('ProjectRuntime staged visibility', () => {
  ...
  it('keeps capability gates disabled by default', () => {
    const runtime = new ProjectRuntime();
    expect(runtime.capabilities).toEqual({ crossMotion: false, freeTracks: false });
    expect(() => runtime.assertCapability('crossMotion')).toThrow(/disabled/);
    expect(() => runtime.assertCapability('freeTracks')).toThrow(/disabled/);
  });

  it('allows explicit opt-in without changing the default runtime', () => {
    const runtime = new ProjectRuntime({ capabilities: { crossMotion: true, freeTracks: true } });
    expect(runtime.capabilities).toEqual({ crossMotion: true, freeTracks: true });
    expect(runtime.assertCapability('crossMotion')).toBe(true);
    expect(runtime.assertCapability('freeTracks')).toBe(true);
  });
