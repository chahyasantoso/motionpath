...
function compareValue(actual, expected, path, tolerance, mismatches) {
  ...
  if (Array.isArray(actual) && Array.isArray(expected)) {
    if (actual.length !== expected.length)
      mismatches.push(
        `${path}: array length ${actual.length} !== ${expected.length}`,
      );
...
export function composeLegacyPatches(graph, tracks) {
  ...
  for (const id of graph.order) {
    ...
    if (track && !track.isDestroyed)
      composed.set(id, track.compose(undefined, composed));
  }
  return composed;
}

export function compareShadowPatches(
  legacy,
  published,
  { tolerance = 1e-8 } = {},
) {
  ...
  for (const id of ids) {
    if (!legacy.has(id)) {
      mismatches.push(`${id}: missing from legacy patches`);
      continue;
    }
    if (!published.has(id)) {
      mismatches.push(`${id}: missing from publisher patches`);
      continue;
    }
...
export function shadowFixture({ graph, tracks, runtime, marks = [] }) {
  ...
  const published = new Map(
    [...runtime.patches.snapshot()].map(([id, patch]) => [id, patch.values]),
  );
  return {
    legacy,
    published,
    comparison: compareShadowPatches(legacy, published),
  };
}
