...
export function tryTopologicalOrder(nodes, edges) {
  ...
  for (const edge of edges) {
    if (!indegree.has(edge.source) || !indegree.has(edge.target))
      throw new Error(
        `Observation edge references an unknown track ('${edge.source}' -> '${edge.target}').`,
      );
  ...
  while (queue.length) {
    ...
    for (const target of outgoing.get(id)) {
      ...
      const insertAt = queue.findIndex(
        (queuedId) => position.get(queuedId) > position.get(target),
      );
...
export function normalizeObservationGraph(motion) {
  ...
  tracks.forEach((track, index) => {
    ...
    if (nodeIndexes.has(id)) {
      errors.push({
        ruleId: "track-observations-duplicate-node",
        path: `tracks[${index}].id`,
    ...
    for (const [edgeIndex, edge] of (track?.observes || []).entries()) {
      ...
      if (typeof edge.source !== "string" || !nodeIndexes.has(edge.source)) {
        errors.push({
          ruleId: "track-observations",
          path: `${path}.source`,
          message: `Unknown source track '${edge.source}'.`,
        });
        continue;
      }
      ...
      const candidate = {
        ...
        target: targetNode,
        role,
        input: role === "input" ? edge.target : undefined,
      };
      const key = observationGraphEdgeKey(candidate);
      if (edgeKeys.has(key)) {
        errors.push({
          ruleId: "track-observations-duplicate-edge",
          path,
          message: `Duplicate observation edge from '${edge.source}' to '${targetNode}'.`,
        });
        continue;
      }
      edgeKeys.add(key);
  ...
  return new ObservationGraph({ nodes, edges, order, errors });
}
export function topologicalTrackOrder(graph, { strict = true } = {}) {
  ...
  if (strict && graph.errors?.length)
    throw new Error(
      `Cannot use invalid observation graph: ${graph.errors.map((error) => error.message).join("; ")}`,
    );
  return [...graph.order];
}
