...
export class ObservationStateBridge {
  ...
  #bindObserverSnapshots() {
    for (const track of this.#tracks.values()) {
      ...
      Object.defineProperty(track, "observerIds", {
        configurable: true,
        get: () => this.#state.getObserverIds(track.id),
      });
    }
  }
  ...
  assertGraphParity(graph) {
    ...
    if (actual.size !== expected.size || [...actual].some((key) => !expected.has(key))) {
      throw new Error("ObservationState mismatch: live graph wiring does not match declared edges in normalized graph IR.");
    }
    return true;
  }
