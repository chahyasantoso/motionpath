...
export class ObservationStateBridge {
  ...
  constructor({ tracks = new Map(), edges } = {}) {
    ...
    this.#controller = new ObservationTrackController({
      state: this.#state,
      tracks: this.#tracks,
    });
      ...
      for (const edge of edges) {
        const source =
          typeof edge.source === "object"
            ? edge.source
            : this.#tracks.get(edge.source);
        if (source && this.#tracks.has(source.id))
          this.#state.addEdge({
            source: source.id,
            target: track.id,
            role: edge.role,
  ...
  #bindObserverSnapshots() {
    for (const track of this.#tracks.values()) {
      ...
      if (typeof track._setObservationObserverIds === "function")
        track._setObservationObserverIds(provider);
      else if (!Object.prototype.hasOwnProperty.call(track, "observerIds"))
        Object.defineProperty(track, "observerIds", {
          configurable: true,
          get: provider,
        });
    }
  ...
  assertGraphParity(graph) {
    ...
    const expected = new Set(
      (graph?.edges ?? []).map((edge) => this.#key(edge)),
    );
    ...
    if (
      ...
      throw new Error(
        "ObservationState mismatch: live graph wiring does not match declared edges in normalized graph IR.",
      );
    return true;
  }
  assertParity() {
    ...
    for (const track of this.#tracks.values())
      for (const edge of this.#state.getEdges(track.id))
        if (!edge.source || !this.#tracks.has(edge.source.id))
          throw new Error(
            "ObservationState contains an edge for an unknown source.",
          );
    return true;
  }
  assertCompositionParity() {
    if (this.#destroyed)
      throw new Error("ObservationStateBridge is destroyed.");
    for (const track of this.#tracks.values()) {
      if (track.isDestroyed || typeof track.composeLocal !== "function")
        continue;
      ...
      if (!patchesEqual(live, shadow))
        throw new Error(
          `ObservationState composition differs for track '${track.id}'.`,
        );
    }
    return true;
  }
