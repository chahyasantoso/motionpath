...
/** ObservationState owner used while authored graph wiring is being externalized. */
export class ObservationStateBridge {
  #tracks; #state; #controller; #destroyed = false; #unsubscribers = [];
  constructor({ tracks = new Map(), edges } = {}) {
    ...
    if (edges) {
      for (const edge of edges) if (this.#tracks.has(edge.source) && this.#tracks.has(edge.target)) this.#state.addEdge(edge);
    } else {
  ...
  get state() { return this.#state; } get controller() { return this.#controller; } get tracks() { return new Map(this.#tracks); }
  #hydrateFromOwners() {
  ...
  #bindObserverSnapshots() {
    for (const track of this.#tracks.values()) {
      if (Object.prototype.hasOwnProperty.call(track, "observerIds")) continue;
      Object.defineProperty(track, "observerIds", { configurable: true, get: () => this.#state.getObserverIds(track.id) });
    }
  }
  #bindSourceCleanup() {
  ...
  assertGraphParity(graph) {
    ...
    const expected = new Set((graph?.edges ?? []).map((edge) => this.#key(edge))); const actual = new Set();
