...
export class ObservationStateBridge {
  ...
  assertCompositionParity() {
    ...
    for (const track of this.#tracks.values()) {
      ...
      const live = track.compose(); const shadow = this.#state.compose(track.id, undefined, new Map(), trackComposeLeaf);
      if (!patchesEqual(live, shadow)) throw new Error(`ObservationState composition differs for track '${track.id}'.`);
    }
    return true;
  }
