...
export class ObservationTrackController {
  ...
  removeObserved(observer, source, { role, target } = {}) {
    this.#state.removeEdge({ source: this.#id(source), target: this.#id(observer), role, input: target });
  }
  ...
  replaceObserved(observer, oldSource, newSource, mapFn, opts = {}) {
    ...
    return this.#state.replaceEdge(
      ...
      { source: this.#id(newSource), target: this.#id(observer), role: opts.role, input: opts.target, mapFn },
    );
  }

  clearObserved(observer) {
    for (const edge of this.getEdges(observer)) this.removeObserved(observer, edge.source, { role: edge.role, target: edge.input });
  }
