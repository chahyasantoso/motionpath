...
export class ObservationTrackController {
  ...
  constructor({ state, tracks = new Map() } = {}) {
    if (!state)
      throw new TypeError(
        "ObservationTrackController requires observation state.",
      );
    this.#state = state;
  ...
  replaceObserved(observer, oldSource, newSource, mapFn, opts = {}) {
    ...
    const oldId = this.#id(oldSource);
    const targetId = this.#id(observer);
    const newId = this.#id(newSource);
    const oldEdges = this.#state
      .getEdges(targetId)
      .filter(
        (edge) =>
          this.#id(edge.source) === oldId &&
    ...
    if (!oldEdges.length) {
      const current = this.#state
        ...
        .filter(
          (edge) =>
            this.#id(edge.source) === newId &&
            (opts.role === undefined || edge.role === opts.role),
        );
      for (const edge of current) {
        observer._emitObservationLifecycle?.({
          type: "edge-removed",
          track: observer,
          source: oldSource,
          edge: {
    ...
    return this.#state.replaceEdge(
      { source: oldId, target: targetId, role: opts.role },
      {
        source: newId,
        target: targetId,
        role: opts.role,
  ...
  removeSourceEdges(source) {
    this.#state.removeSourceEdges(this.#id(source));
  }
