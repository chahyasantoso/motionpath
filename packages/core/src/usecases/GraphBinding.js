...
import { hasMutationOverride } from "./LegacyObservationFacade.js";

/** Transactional bridge with ObservationState as the sole authored graph owner. */
export class GraphBinding {
  ...
  constructor({
    ...
    const ownerEdges = initialEdges.length
      ? initialEdges
      : this.#captureTrackEdges();
    this.#observationBridge = new ObservationStateBridge({
      tracks: this.#tracks,
      edges: ownerEdges,
    });
  ...
  get observationState() {
    return this.#observationBridge?.state ?? null;
  }
  get isDestroyed() {
  ...
  #captureTrackEdges() {
    ...
    for (const track of this.#tracks.values())
      for (const edge of track.getObservationOwner?.()?.getEdges(track) ?? [])
        edges.push({
  ...
  #bindObservationController() {
    ...
    for (const track of this.#tracks.values())
      track._setObservationController?.(controller);
  }
  #controller() {
    return this.#observationBridge.controller;
  }
  replaceEdge(oldEdge, newEdge) {
    ...
    if (!observer || !oldSource || !newSource)
      throw new Error("replaceEdge references an unknown track.");
    const candidate = this.#candidateGraph((edges) => [
      ...edges.filter(
        (edge) =>
          !observationEdgeEquals(edge, { ...oldEdge, target: observer.id }),
      ),
      this.#normalizeEdge({ ...newEdge, target: observer.id }),
    ]);
    const role = newEdge.role ?? oldEdge.role;
    const input =
      role === "input" ? (newEdge.input ?? newEdge.target) : undefined;
    const replaced = this.#edgeSnapshots(observer, oldSource, {
      role: oldEdge.role,
    });
    this.#transaction(
      () => {
      ...
      () => {
        const original = replaced[0];
        if (!original) return;
        this.#observationBridge.state.removeEdge({
          source: newSource.id,
          target: observer.id,
          role,
          input,
  ...
  addEdge(edge) {
    ...
    if (!observer || !source)
      throw new Error("addEdge references an unknown track.");
    const candidate = this.#candidateGraph((edges) => [
      ...edges,
      this.#normalizeEdge(edge),
    ]);
    ...
    this.#transaction(
      () =>
        this.#observationBridge.state.addEdge({
          ...
          role,
          input,
          mapFn: edge.mapFn ?? null,
        }),
      () =>
        previous
          ? this.#restoreEdge(observer, source, previous)
          : this.#detachEdge(observer, source, { role, input }),
      candidate,
    );
  }
  removeEdge(edge) {
    ...
    const candidate = this.#candidateGraph((edges) =>
      edges.filter(
        (existing) =>
          !observationEdgeEquals(existing, this.#normalizeEdge(edge)),
      ),
    );
    const removed = this.#edgeSnapshots(observer, source, {
      role: edge.role,
      input: edge.input,
    });
    this.#transaction(
      ...
      () => {
        for (const original of removed)
          this.#restoreEdge(observer, source, original);
      },
      candidate,
    );
  }
  addTrack(track, observesOrOptions = []) {
    this.#assertAlive();
    const observes = Array.isArray(observesOrOptions)
      ? observesOrOptions
      : (observesOrOptions.observes ?? []);
    if (!track?.id)
      throw new TypeError("GraphBinding.addTrack requires a Track with an id.");
    ...
    const candidate = this.#candidateGraph(
      (edges) => [
        ...edges,
        ...observes.map((edge) =>
          this.#normalizeEdge({ ...edge, target: track.id }),
        ),
      ],
    ...
    const resolved = observes.map((edge) => {
      ...
      return {
        source,
        role,
        input: role === "input" ? (edge.input ?? edge.target) : undefined,
        mapFn: edge.mapFn ?? null,
      };
    });
    ...
    try {
      for (const { source, role, input, mapFn } of resolved) {
        undo.push(() => this.#detachEdge(track, source, { role, input }));
        if (hasMutationOverride(track, "setObserved"))
          track.setObserved(source, mapFn, { role, target: input });
        else
          this.#controller().setObserved(track, source, mapFn, {
            role,
            target: input,
          });
      }
  ...
  #edgeSnapshots(observer, source, { role, input } = {}) {
    return this.#observationBridge.state
      .getEdges(observer.id)
      .filter(
        (existing) =>
  ...
  #wireInitialEdges(edges) {
    ...
    try {
      for (const edge of edges) {
        ...
        if (!observer || !source)
          throw new Error(
            "GraphBinding initial edge references an unknown track.",
          );
        const role = edge.role ?? "output";
        const input =
          role === "input" ? (edge.input ?? edge.target) : undefined;
        this.#controller().setObserved(observer, source, edge.mapFn ?? null, {
          role,
          target: input,
        });
  ...
  #freeze(graph) {
    if (!graph || graph.errors?.length)
      throw new Error("GraphBinding requires a valid normalized graph.");
    topologicalTrackOrder(graph, { strict: true });
    return Object.freeze({
      valid: true,
      nodes: toImmutableList(graph.nodes.map((node) => ({ ...node }))),
      edges: toImmutableList(
        graph.edges.map(({ source, target, role, input }) => ({
          source,
          target,
          role,
          input,
  ...
  #assertTrackGraphMatches() {
    ...
    for (const id of declaredNodes)
      if (!this.#tracks.has(id))
        throw new Error(
          `GraphBinding graph declares node '${id}' with no live Track.`,
        );
    for (const id of this.#tracks.keys())
      if (!declaredNodes.has(id))
        throw new Error(
          `GraphBinding holds Track '${id}' that the graph does not declare.`,
        );
    this.#observationBridge.assertGraphParity(this.#graph);
  }
  #candidateGraph(edgeMutator, nodeMutator = (nodes) => nodes) {
    const nodes = nodeMutator(
      this.#graph.nodes.map((node) => ({ id: node.id })),
    );
      ...
      tracks: nodes.map((node) => ({
        ...
        observes: edges
          ...
          .map((edge) => ({
            source: edge.source,
            role: edge.role,
            target: edge.role === "input" ? edge.input : undefined,
          })),
      })),
    });
    if (!normalized.valid)
      throw new Error(
        `Rejected graph mutation: ${normalized.errors.map((error) => error.message).join("; ")}`,
      );
    return normalized;
  }
  #commit(graph) {
  ...
  #syncPublisher() {
    this.#publisher.applyGraph(this.#graph, this.#tracks);
  }
  #subscribe() {
    for (const track of this.#tracks.values()) this.#subscribeTrack(track);
  }
  #subscribeTrack(track) {
    const unsubscribe = track.onLifecycle?.((event) => {
      if (this.#destroyed) return;
      if (event.type === "invalidated") this.#publisher.markDirty(track.id);
      if (event.type === "destroyed") this.removeTrack(track.id);
      if (event.type === "detached")
        this.removeTrack(track.id, { destroy: false });
    });
    if (unsubscribe) this.#unsubscribers.push(unsubscribe);
  }
  #assertAlive() {
    if (this.#destroyed) throw new Error("GraphBinding is destroyed.");
  }
}
