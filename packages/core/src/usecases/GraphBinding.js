...
/** Transactional graph binding with one owner-facing mutation path. */
export class GraphBinding {
  ...
  constructor({ graph, tracks = new Map(), publisher, ownsPublisher = true, initialEdges = [] } = {}) {
    ...
    const ownerEdges = initialEdges.length ? initialEdges : this.#captureTrackEdges();
    this.#observationBridge = new ObservationStateBridge({ tracks: this.#tracks, edges: ownerEdges });
    this.#bindObservationController();
  ...
  #captureTrackEdges() { const edges = []; for (const track of this.#tracks.values()) for (const edge of track.getObservationOwner?.()?.getEdges(track) ?? []) edges.push({ source: edge.source.id, target: track.id, role: edge.role, input: edge.input, mapFn: edge.mapFn }); return edges; }
  #bindObservationController() { const controller = this.#observationBridge.controller; for (const track of this.#tracks.values()) track._setObservationController?.(controller); }
  #controller() { return this.#observationBridge.controller; }
  replaceEdge(oldEdge, newEdge) { this.#assertAlive(); const observer = this.#tracks.get(oldEdge.target ?? newEdge.target); const oldSource = this.#tracks.get(oldEdge.source); const newSource = this.#tracks.get(newEdge.source); if (!observer || !oldSource || !newSource) throw new Error("replaceEdge references an unknown track."); const candidate = this.#candidateGraph((edges) => [...edges.filter((edge) => !observationEdgeEquals(edge, { ...oldEdge, target: observer.id })), this.#normalizeEdge({ ...newEdge, target: observer.id })]); const role = newEdge.role ?? oldEdge.role; const input = role === "input" ? (newEdge.input ?? newEdge.target) : undefined; const replaced = this.#edgeSnapshots(observer, oldSource, { role: oldEdge.role }); this.#transaction(() => this.#observationBridge.state.replaceEdge({ source: oldSource.id, target: observer.id, role: oldEdge.role }, { source: newSource.id, target: observer.id, role, input, mapFn: newEdge.mapFn }), () => { const original = replaced[0]; if (!original) return; this.#observationBridge.state.removeEdge({ source: newSource.id, target: observer.id, role, input }); this.#observationBridge.state.addEdge({ source: oldSource.id, target: observer.id, role: original.role, input: original.input, mapFn: original.mapFn }); }, candidate); }
  addEdge(edge) { this.#assertAlive(); const observer = this.#tracks.get(edge.target); const source = this.#tracks.get(edge.source); if (!observer || !source) throw new Error("addEdge references an unknown track."); const candidate = this.#candidateGraph((edges) => [...edges, this.#normalizeEdge(edge)]); const role = edge.role ?? "output"; const input = role === "input" ? (edge.input ?? edge.target) : undefined; const [previous] = this.#edgeSnapshots(observer, source, { role, input }); this.#transaction(() => this.#observationBridge.state.addEdge({ source: source.id, target: observer.id, role, input, mapFn: edge.mapFn ?? null }), () => previous ? this.#restoreEdge(observer, source, previous) : this.#detachEdge(observer, source, { role, input }), candidate); }
  removeEdge(edge) { this.#assertAlive(); const observer = this.#tracks.get(edge.target); const source = this.#tracks.get(edge.source); if (!observer || !source) return; const candidate = this.#candidateGraph((edges) => edges.filter((existing) => !observationEdgeEquals(existing, this.#normalizeEdge(edge)))); const removed = this.#edgeSnapshots(observer, source, { role: edge.role, input: edge.input }); this.#transaction(() => this.#observationBridge.state.removeEdge({ source: source.id, target: observer.id, role: edge.role, input: edge.input }), () => { for (const original of removed) this.#restoreEdge(observer, source, original); }, candidate); }
  addTrack(track, observesOrOptions = []) { this.#assertAlive(); const observes = Array.isArray(observesOrOptions) ? observesOrOptions : (observesOrOptions.observes ?? []); if (!track?.id) throw new TypeError("GraphBinding.addTrack requires a Track with an id."); if (this.#tracks.has(track.id)) throw new Error(`Duplicate track id '${track.id}'.`); const candidate = this.#candidateGraph((edges) => [...edges, ...observes.map((edge) => this.#normalizeEdge({ ...edge, target: track.id }))], (nodes) => [...nodes, { id: track.id }]); const resolved = observes.map((edge) => { const source = this.#tracks.get(edge.source); if (!source) throw new Error(`Unknown source track '${edge.source}'.`); const role = edge.role ?? "output"; return { source, role, input: role === "input" ? (edge.input ?? edge.target) : undefined, mapFn: edge.mapFn ?? null }; }); const previousGraph = this.#graph; const undo = []; this.#tracks.set(track.id, track); this.#observationBridge.state.register(track); this.#bindObservationController(); try { for (const { source, role, input, mapFn } of resolved) { undo.push(() => this.#detachEdge(track, source, { role, input })); this.#controller().setObserved(track, source, mapFn, { role, target: input }); } this.#commit(candidate); this.#subscribeTrack(track); } catch (error) { this.#unwind(undo); this.#observationBridge.state.unregister(track.id, { detach: false }); this.#tracks.delete(track.id); this.#graph = previousGraph; this.#refreshObservationBridge(); throw error; } }
  ...
  destroy() { if (this.#destroyed) return; this.#destroyed = true; for (const unsubscribe of this.#unsubscribers) unsubscribe(); this.#unsubscribers = []; this.#tracks = new Map(); this.#observationBridge?.destroy(); this.#observationBridge = null; const publisher = this.#publisher; this.#publisher = null; if (this.#ownsPublisher) publisher?.destroy?.(); }
  #edgeSnapshots(observer, source, { role, input } = {}) { return this.#observationBridge.state.getEdges(observer.id).filter((existing) => existing.source === source && (role === undefined || existing.role === role) && (role !== "input" || input === undefined || existing.input === input)); }
  #restoreEdge(observer, source, snapshot) { this.#observationBridge.state.addEdge({ source: source.id, target: observer.id, role: snapshot.role, input: snapshot.input, mapFn: snapshot.mapFn }); }
  #detachEdge(observer, source, { role, input }) { this.#observationBridge.state.removeEdge({ source: source.id, target: observer.id, role, input }); }
  #wireInitialEdges(edges) { const undo = []; try { for (const edge of edges) { const observer = this.#tracks.get(edge.target); const source = this.#tracks.get(edge.source); if (!observer || !source) throw new Error("GraphBinding initial edge references an unknown track."); const role = edge.role ?? "output"; const input = role === "input" ? (edge.input ?? edge.target) : undefined; this.#controller().setObserved(observer, source, edge.mapFn ?? null, { role, target: input }); undo.push(() => this.#detachEdge(observer, source, { role, input })); } this.#observationBridge.assertParity(); } catch (error) { for (const rollback of undo.reverse()) rollback(); this.#refreshObservationBridge(); throw error; } }
  ...
  #commit(graph) { const frozen = this.#freeze(graph); this.#publisher.applyGraph(frozen, this.#tracks); this.#graph = frozen; this.#refreshObservationBridge(); this.#observationBridge.assertGraphParity(this.#graph); }
  #refreshObservationBridge() { if (!this.#observationBridge) return; const edges = []; for (const track of this.#tracks.values()) for (const edge of this.#observationBridge.state.getEdges(track.id)) edges.push({ source: edge.source.id, target: track.id, role: edge.role, input: edge.input, mapFn: edge.mapFn }); this.#observationBridge.destroy(); this.#observationBridge = new ObservationStateBridge({ tracks: this.#tracks, edges }); this.#bindObservationController(); }
  ...
  #subscribeTrack(track) { const unsubscribe = track.onLifecycle?.((event) => { if (this.#destroyed) return; if (event.type === "invalidated") this.#publisher.markDirty(track.id); if (event.type === "destroyed") this.removeTrack(track.id); if (event.type === "detached") this.removeTrack(track.id, { destroy: false }); }); if (unsubscribe) this.#unsubscribers.push(unsubscribe); }
