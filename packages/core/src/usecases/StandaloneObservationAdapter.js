...
export class StandaloneObservationAdapter {
  #owner = sharedOwner; #keys = new WeakMap(); #tracks = new Map(); #sourceUnsubscribers = new Map(); #lifecycleUnsubscribers = new Map(); #destroyed = false;
  constructor({ tracks = [] } = {}) { const registry = tracks instanceof Map ? [...tracks.values()] : tracks; for (const track of registry) this.register(track); }
  get state() { return { tracks: new Map(this.#tracks), getEdges: (target) => this.#stateEdges(target), getSources: (target) => this.#stateSources(target), getObserverIds: (source) => this.getObserverIds(source) }; }
  ...
  setObserved(observer, source, mapFn, { role = "output", target } = {}) { this.#assertAlive(); this.register(observer); this.register(source); return this.#owner.addEdge({ source: this.#keys.get(source), target: this.#keys.get(observer), role, input: role === "input" ? (target ?? observer.id) : undefined, mapFn: mapFn ?? null }); }
  ...
  compose(track, rawData, ctx) { this.#assertAlive(); this.register(track); const internal = this.#internalContext(ctx); const patch = this.#owner.compose(this.#keys.get(track), rawData, internal); if (ctx && internal !== ctx) ctx.set(track.id, patch); return patch; }
  destroy() { if (this.#destroyed) return; for (const track of [...this.#tracks.values()]) this.#unregister(track); this.#destroyed = true; this.#sourceUnsubscribers.clear(); this.#lifecycleUnsubscribers.clear(); }
  #unregister(trackOrId) { const track = typeof trackOrId === "object" ? trackOrId : this.#findTrack(trackOrId); const key = track ? this.#keys.get(track) : undefined; if (!key) return; this.#sourceUnsubscribers.get(track)?.(); this.#sourceUnsubscribers.delete(track); this.#lifecycleUnsubscribers.get(track)?.(); this.#lifecycleUnsubscribers.delete(track); const refs = (globalRefs.get(key) ?? 1) - 1; if (refs <= 0) { this.#owner.unregister(key); globalRefs.delete(key); globalTracks.delete(key); globalKeys.delete(track); } else globalRefs.set(key, refs); this.#tracks.delete(key); this.#keys.delete(track); }
  #stateEdges(targetOrTrack) { const track = this.#resolveTrack(targetOrTrack); return track ? this.getEdges(track) : []; }
  #stateSources(targetOrTrack) { const track = this.#resolveTrack(targetOrTrack); return track ? this.#owner.getSources(this.#keys.get(track)) : []; }
  #internalContext(ctx) { if (!ctx) return new Map(); if (internalContexts.has(ctx)) return ctx; let internal = publicContexts.get(ctx); if (!internal) { internal = new Map(); publicContexts.set(ctx, internal); internalContexts.add(internal); for (const [publicId, patch] of ctx) { const track = this.#findTrack(publicId); if (track) internal.set(globalKeys.get(track), patch); } } return internal; }
