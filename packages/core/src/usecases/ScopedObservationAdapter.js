...
export class ScopedObservationAdapter {
  ...
  get state() {
    return { tracks: new Map(this.#tracks), getEdges: (target) => this.getEdges(target), getSources: (target) => this.#stateSources(target), getObserverIds: (source) => this.getObserverIds(source) };
  }
  ...
  unregister(trackOrId) { if (!this.#destroyed) this.#unregister(trackOrId); }
  ...
  getEdges(targetOrTrack) {
    ...
    if (!track) return [];
    return this.#owner.getEdges(this.#keys.get(track)).map((edge) => ({ ...edge, source: this.#owner.getTrack(edge.source) ?? edge.source, target: track.id }));
  }
  ...
  getObserverIds(sourceOrTrack) {
    ...
    if (!source) return [];
    return this.#owner.getObserverIds(this.#keys.get(source)).map((key) => this.#owner.getTrack(key)?.id).filter(Boolean);
  }
  ...
  clearObserved(observer) {
    ...
    if (!target) return;
    for (const edge of this.#owner.getEdges(target)) this.#owner.removeEdge({ source: this.#keys.get(edge.source), target, role: edge.role, input: edge.input });
  }
  ...
  #unregister(trackOrId) {
    ...
    if (this.#tracksById.get(track.id) === track) {
      ...
      for (const candidate of this.#tracks.values()) if (candidate.id === track.id) { this.#tracksById.set(track.id, candidate); break; }
    }
  ...
  #watchTrack(track) {
    ...
    if (typeof track.onSourceDestroyed === "function") this.#sourceUnsubscribers.set(track, track.onSourceDestroyed((event) => { if (!event || !Array.isArray(event.observerIds)) return; event.observerIds.splice(0, event.observerIds.length, ...this.getObserverIds(track)); }));
  }
