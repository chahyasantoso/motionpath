...
export class ObservationState {
  ...
  constructor({
    ...
    this.#onInvalidate =
      typeof onInvalidate === "function" ? onInvalidate : () => {};
    this.#validateCycles = validateCycles !== false;
    for (const [id, track] of tracks instanceof Map ? tracks : new Map(tracks))
  ...
  register(track, id = track?.id) {
    ...
    if (!track?.id || !id)
      throw new TypeError(
        "ObservationState.register requires a track with an id.",
      );
    ...
    if (!this.#observers.has(id)) this.#observers.set(id, new Set());
    if (!this.#idsByTrack.has(track)) this.#idsByTrack.set(track, id);
    return track;
  }
  ...
  removeSourceEdges(sourceId) {
    ...
    for (const targetId of [...(this.#observers.get(sourceId) ?? [])]) {
      ...
      for (const edge of [...targetEdges.values()])
        if (
          edge.source.id === this.#tracks.get(sourceId)?.id ||
          edge.source === this.#tracks.get(sourceId)
        )
          this.removeEdge({
            source: sourceId,
            target: targetId,
  ...
  getEdges(targetId) {
    return [...(this.#edges.get(targetId)?.values() ?? [])].map((edge) => ({
      ...edge,
    }));
  }
  getTrack(id) {
  ...
  getSources(targetId) {
    return [...new Set(this.getEdges(targetId).map(({ source }) => source.id))];
  }
  getObserverIds(sourceId) {
    return [...(this.#observers.get(sourceId) ?? [])];
  }
  addEdge({ source, target, role = "output", input, mapFn = null }) {
    ...
    this.#invalidate({
      ...
      edge: { source, target, role, input: normalizedInput },
    });
    return this.getEdges(target).find(
      (edge) =>
        observationEdgeKey(edge.source.id, edge.role, edge.input) === key,
    );
  }
  removeEdge({ source, target, role, input }) {
    ...
    if (!bucket) return;
    const keys = [...bucket.entries()]
      .filter(
        ([, edge]) =>
          edge.source === this.#tracks.get(source) &&
          (role === undefined || edge.role === role) &&
          (role !== "input" || input === undefined || edge.input === input),
      )
      .map(([key]) => key);
    ...
    if (
      keys.length &&
      ![...bucket.values()].some(
        (edge) => edge.source === this.#tracks.get(source),
      )
    )
      this.#observers.get(source)?.delete(target);
    if (keys.length)
      this.#invalidate({
        type: "edge-removed",
        edge: { source, target, role, input },
      });
  }
  replaceEdge(oldEdge, newEdge) {
    this.#assertAlive();
    const old = this.getEdges(oldEdge.target).find(
      (edge) =>
        edge.source === this.#tracks.get(oldEdge.source) &&
        (oldEdge.role === undefined || edge.role === oldEdge.role),
    );
    if (!old)
      throw new Error(
        `Track "${oldEdge.target}" does not observe "${oldEdge.source}".`,
      );
    const next = {
      source: newEdge.source,
      target: oldEdge.target,
      role: newEdge.role ?? old.role,
      input: newEdge.input ?? old.input,
      mapFn: newEdge.mapFn ?? old.mapFn,
    };
  ...
  compose(targetId, rawData, ctx = new Map(), composeLeaf, composeSource) {
    ...
    if (typeof composeLeaf !== "function")
      throw new TypeError("ObservationState.compose requires composeLeaf.");
    ...
    const composeObserved = (source) =>
      composeSource
        ? composeSource(source, ctx)
        : this.compose(
            this.#keyForTrack(source),
            undefined,
            ctx,
            composeLeaf,
            composeSource,
  ...
  #edgeList(targetId) {
    return [...(this.#edges.get(targetId)?.values() ?? [])];
  }
  #keyForTrack(track) {
    const cached = this.#idsByTrack.get(track);
    if (cached !== undefined && this.#tracks.get(cached) === track)
      return cached;
    for (const [id, value] of this.#tracks)
      if (value === track) {
        this.#idsByTrack.set(track, id);
        return id;
      }
    return track.id;
  }
  #assertAcyclic(target, source) {
    ...
    while (queue.length) {
      ...
      if (id === target)
        throw new Error(
          `Observing "${source}" from "${target}" would create a cycle.`,
        );
      if (seen.has(id)) continue;
      seen.add(id);
      for (const edge of this.#edgeList(id))
        queue.push(this.#keyForTrack(edge.source));
    }
  }
  #invalidate(event) {
    if (!this.#destroyed) this.#onInvalidate(event);
  }
  #assertAlive() {
    if (this.#destroyed) throw new Error("ObservationState is destroyed.");
  }
}
