...
export class ObservationState {
  ...
  getSources(targetId) { return [...new Set(this.getEdges(targetId).map(({ source }) => source.id))]; } getObserverIds(sourceId) { return [...(this.#observers.get(sourceId) ?? [])]; }
  ...
  #keyForTrack(track) { const cached = this.#idsByTrack.get(track); if (cached !== undefined && this.#tracks.get(cached) === track) return cached; for (const [id, value] of this.#tracks) if (value === track) { this.#idsByTrack.set(track, id); return id; } return track.id; }
