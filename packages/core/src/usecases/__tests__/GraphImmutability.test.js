...
describe("P2-01 graph value immutability", () => {
  ...
  it("rejects mutation of a normalized graph through any reachable path", () => {
    ...
    expect(() => {
      graph.nodes.push({ id: "c" });
    }).toThrow(TypeError);
    expect(() => {
      graph.edges[0].source = "hacked";
    }).toThrow(TypeError);
    expect(() => {
      graph.order.push("c");
    }).toThrow(TypeError);
  ...
  it("freezes diagnostics on an invalid graph", () => {
    const graph = normalizeObservationGraph({
      tracks: [{ id: "a", observes: [{ source: "missing" }] }],
    });
  ...
  it("hands GraphBinding consumers a deeply frozen snapshot, not a frozen wrapper", () => {
    ...
    try {
      ...
      expect(() => {
        snapshot.nodes.push({ id: "injected" });
      }).toThrow(TypeError);
      expect(() => {
        snapshot.edges[0].target = "hacked";
      }).toThrow(TypeError);
      expect(() => {
        snapshot.order.push("injected");
      }).toThrow(TypeError);
    } finally {
      binding.destroy();
    }
