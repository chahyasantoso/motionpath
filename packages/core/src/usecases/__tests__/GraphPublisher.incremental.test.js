...
describe("GraphPublisher incremental cache", () => {
  ...
  it("keeps an independent branch publishing when another branch fails to publish", () => {
    const { graph, tracks, composeCounts } = buildRealGraph(
      independentChainsMotion(),
    );
    let fail = true;
    const published = [];
  ...
  it("stops a node after its bounded publish attempts are exhausted", () => {
    ...
    const publisher = new GraphPublisher({
      ...
      publish: (id) => {
        if (id === "n0") {
          attempts += 1;
          throw new Error("permanent renderer failure");
        }
        published.push(id);
      },
    });
  ...
  it("does not spend publish retry budget on compose failures", () => {
    ...
    const { graph, tracks } = buildRealGraph(chainMotion(1), {
      onCompose: () => {
        if (failCompose) throw new Error("bad state");
      },
    });
    let attempts = 0;
    const publisher = new GraphPublisher({
