...
describe("GraphPublisher — single compose-context key space (D2)", () => {
  ...
  it("Motion.composeGraph returns exactly one entry per track, keyed by id", () => {
    const { graph, tracks } = buildRealGraph(diamondMotion());
    ...
    motion.init();
    for (const track of tracks.values()) motion.mount(track, 0);
    const patches = motion.composeGraph();
    expect(patches.size).toBe(4);
    expect([...patches.keys()].sort()).toEqual(["a", "b", "c", "d"]);
    for (const key of patches.keys()) expect(typeof key).toBe("string");
    motion.destroy();
  });
});
