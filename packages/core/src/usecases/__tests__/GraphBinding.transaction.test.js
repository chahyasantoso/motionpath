...
function edgeKey(source, target, role, input) { return `${source}|${role ?? "output"}|${input ?? ""}->${target}`; }
...
function declaredEdgeKeys(graph) { return graph.edges.map((edge) => edgeKey(edge.source, edge.target, edge.role, edge.input)).sort(); }

function ownerEdgeKeys(binding) {
  const keys = [];
  for (const [target] of binding.tracks) {
    for (const edge of binding.observationState.getEdges(target)) keys.push(edgeKey(edge.source.id, target, edge.role, edge.input));
  }
  return keys.sort();
}

function observerCounts(tracks) { return Object.fromEntries([...tracks].map(([id, track]) => [id, track.observerCount])); }
...
function snapshot(binding, publisher) {
  return { order: publisher.graphOrder, declared: declaredEdgeKeys(binding.graph), live: liveEdgeKeys(binding.tracks), observers: observerCounts(binding.tracks), nodes: binding.graph.nodes.map((node) => node.id).sort() };
}
...
describe("GraphBinding: addTrack rolls back at every failing edge index", () => {
  ...
  for (let failAt = 0; failAt < observes.length; failAt += 1) {
    it(`restores the exact prior state when wiring fails on edge ${failAt}`, () => {
      ...
      let calls = 0;
      late.setObserved = (...args) => { if (calls++ === failAt) throw new Error(`wire failed at ${failAt}`); return realSetObserved(...args); };
...
describe("GraphBinding: rolls back when the publisher rejects the commit", () => {
  function failNextApply(publisher) {
    ...
    let armed = true;
    publisher.applyGraph = (...args) => { if (armed) { armed = false; throw new Error("publisher rejected the candidate"); } return real(...args); };
  }
  ...
  it("puts a rewired observer back on its original source", () => {
    ...
    const before = snapshot(binding, publisher);
    failNextApply(publisher);
    expect(() => binding.replaceEdge({ source: "n1", target: "n2", role: "output" }, { source: "n0", target: "n2", role: "output", mapFn: (patch) => ({ from_n0: patch.transform }) })).toThrow(/publisher rejected/);
...
describe("GraphBinding: cycle rejection is failure-atomic", () => {
  it("preserves owner state, IR, publisher order, and live composition", () => {
    ...
    expect(() => binding.addEdge({ source: "n2", target: "n0", role: "output", mapFn: (patch) => ({ from_n2: patch.transform }) })).toThrow(/cycle/i);
