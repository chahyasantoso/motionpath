...
/**
 * Explicit observation owner used by the Track extraction.
 *
 * This is deliberately independent of Track's lifecycle and playhead state.
 * Standalone owners disable cycle validation to preserve legal mutual
 * observation; authored owners enable it and are wired by GraphBinding.
 */
export class TrackObservationOwner {
  ...
  compose(targetId, rawData, ctx) {
    this.#assertAlive();
    return this.#state.compose(targetId, rawData, ctx, trackComposeLeaf, this.#composeSource);
  }
