...
describe("track observation graph validation", () => {
  it("rejects a two-track cycle", () => {
    const errors = validateProject(
      project([track("a", [{ source: "b" }]), track("b", [{ source: "a" }])]),
    );
    expect(
      errors.some((error) => error.ruleId === "track-observations-cycle"),
    ).toBe(true);
  });
  ...
  it("rejects duplicate and invalid observation edges", () => {
    ...
    expect(
      errors.filter((error) => error.ruleId === "track-observations").length,
