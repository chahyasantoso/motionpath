...
describe("validateProject integration tests", () => {
          ...
          tracks: [
            {
              ...
              keyframes: {
                ...
                }, // rule: path-xy-exclusivity (path & x present)
              },
            },
          ],
        },
  ...
  it("track rules use the 3rd argument positionally as context, never sniffing its type", () => {
    ...
    const errorsXY = pathXYExclusivityRule(
      trackXY,
      motion,
      "not-a-context-object",
      realPath,
  ...
  it("should validate malformed stop shape during full project validation", () => {
    ...
    expect(stopShapeErrors[0].path).toBe(
      "motions[0].tracks[0].keyframes.x.stops[0]",
    );
    expect(stopShapeErrors[1].path).toBe(
      "motions[0].tracks[0].keyframes.x.stops[1]",
    );
  });
});
