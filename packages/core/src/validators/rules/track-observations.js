  ...
  motions.forEach((motion, motionIndex) => {
    ...
    motion.tracks.forEach((track, trackIndex) => {
      ...
      if (!Array.isArray(observations)) {
        errors.push({
          ruleId: "track-observations",
          severity: "error",
          message: "track.observes must be an array.",
          path,
        });
        return;
      }
      ...
      observations.forEach((edge, edgeIndex) => {
        ...
        if (typeof edge.source !== "string" || edge.source.length === 0) {
          errors.push({
            ruleId: "track-observations",
            severity: "error",
            message: "Observation source must be a non-empty track id.",
          ...
          if (edge.source === track.id)
            errors.push({
              ruleId: "track-observations",
              severity: "error",
              message: "A track cannot observe itself.",
              path: `${edgePath}.source`,
            });
          if (!trackIds.has(edge.source))
            errors.push({
              ruleId: "track-observations",
              severity: "error",
              message: `Observed source '${edge.source}' is not a track in this motion.`,
              path: `${edgePath}.source`,
            });
          if (seenSources.has(edge.source))
            errors.push({
              ruleId: "track-observations",
              severity: "error",
              message: `Track observes '${edge.source}' more than once.`,
              path: `${edgePath}.source`,
            });
          seenSources.add(edge.source);
          if (trackIds.has(edge.source) && edge.source !== track.id)
            graph
              .get(track.id)
              .push({ source: edge.source, path: `${edgePath}.source` });
        }
        ...
        if (role === "input") {
          if (typeof edge.target !== "string" || edge.target.length === 0)
            errors.push({
              ruleId: "track-observations",
              severity: "error",
              message: "Input observations require a non-empty target key.",
          ...
          errors.push({
            ...
            severity: "error",
            message:
              "Output observations cannot define target; their patch is merged directly.",
            path: `${edgePath}.target`,
          });
        }
      });
    ...
    const visit = (id, stack = []) => {
      if (visiting.has(id)) {
        ...
        errors.push({
          ruleId: "track-observations-cycle",
          severity: "error",
          message: `Observation cycle detected: ${cycle}.`,
          path: `motions[${motionIndex}].tracks`,
        });
        return;
      }
