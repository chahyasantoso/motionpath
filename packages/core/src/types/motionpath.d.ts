...
export interface PluginRegistry {
  readonly plugins: AnimationPlugin[];
  readonly internalKeys: ReadonlySet<string>;
  readonly inputs: ReadonlyMap<string, AnimationPlugin>;
  ...
  resolveInput(key: string): AnimationPlugin | undefined;
  ensureLoaded(plugin: AnimationPlugin): Promise<void>;
}
export interface TriggerDelegateFactory {
  (config: Trigger): unknown;
}
export interface TriggerDelegateRegistry {
  get(type: Trigger["type"] | string): TriggerDelegateFactory | undefined;
  has(type: string): boolean;
  set(type: string, factory: TriggerDelegateFactory): unknown;
  delete(type: string): boolean;
  entries(): IterableIterator<[string, TriggerDelegateFactory]>;
}
...
export interface RuntimeDependencies {
  ...
  triggerDelegates: TriggerDelegateRegistry;
}
export interface EngineOptions extends Partial<RuntimeDependencies> {
  dependencies?: RuntimeDependencies;
  publisherRendering?: boolean;
  observationOwnership?: ObservationOwnership;
  clock?: {
    subscribe(
      listener: (event: { tick?: number; delta?: number }) => void,
    ): () => void;
    dispose?(): void;
  };
}
export declare class Engine {
  ...
  readonly observationOwnership: ObservationOwnership;
  publisherRendering: boolean;
  loadProject(
...
export declare function resolvePluginInput(
  key: string,
): AnimationPlugin | undefined;
