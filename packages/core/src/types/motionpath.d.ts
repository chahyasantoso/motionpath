...
export interface EngineOptions extends Partial<RuntimeDependencies> { dependencies?: RuntimeDependencies; }
export declare class Engine { constructor(options?: EngineOptions); readonly validationReport: readonly { ruleId: string; severity: string; message: string; path?: string; }[]; readonly instanceCount: number; readonly templates: readonly MotionTemplate[]; loadProject(project: MotionProject, options?: { validate?: boolean }): Promise<void>; mountInstance(id: string): Motion | Track; mountWithDelegate(id: string, delegate: unknown): Motion; createTrackInstance(id: string, overrides?: Partial<MotionTrack>): Track; createMotionHost(options: { id: string; staggerTransition?: StaggerTransition; autoplay?: boolean }): { motion: Motion; track: Track }; adopt<T extends Motion | Track>(object: T): T; unmount(object: Motion | Track): boolean; isOwned(object: Motion | Track): boolean; getTrack(id: string): Track | null; getTrackConfig(id: string): MotionTrack | null; destroy(): void; }
