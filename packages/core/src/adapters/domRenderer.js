...
function normalizePatch(patch) {
  ...
  for (const [key, serialize] of getOutputSerializers()) { const value = normalized[key]; if (value !== null && typeof value === "object" && typeof serialize === "function") normalized[key] = serialize(value); }
...
export function domRenderer(target, patch) {
  ...
  lastApplied.set(target, next); gsap.set(target, dirty);
}
