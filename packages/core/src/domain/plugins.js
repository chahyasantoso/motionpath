...
function validatePlugin(plugin) {
  ...
  for (const field of ["keys", "inputs", "internalKeys"])
    if (
      ...
      (!Array.isArray(plugin[field]) ||
        plugin[field].some((key) => typeof key !== "string"))
    )
  ...
  if (
    ...
    throw new TypeError(
      "Lazy plugins must provide a load() function when specified.",
    );
...
export function createPluginRegistry(initialPlugins = ALL_PLUGINS) {
  ...
  const register = (plugin) => {
    ...
    for (const key of plugin.inputs || []) {
      ...
      if (prior && prior !== plugin)
        throw new Error(`Plugin input collision for "${key}".`);
    }
    plugins.push(plugin);
...
export function resolvePluginInput(key) {
  return typeof key === "string"
    ? defaultRegistry.resolveInput(key)
    : undefined;
}
