  ...
  return (
    <div className="app zuma-app">
      <header className="header zuma-header">
        <h1>
          Zuma <span className="spiral-accent">Graph Flow</span>
        </h1>
        <p className="subtitle">
          GraphPublisher • Explicit Overlay Dependencies • Native Reflow
        </p>
      </header>
      <section className="spiral-scene">
        <div className="scene-label">
          <h2>
            Endless Spiral <span className="spiral-accent">· Graph Flow</span>
          </h2>
          <p>
            Same Zuma behavior, with each ball's path and transition overlays
            published through an explicit graph.
          </p>
        </div>
        <div className="spiral-stage">
          ...
          {ballVms.map((ball) => (
            <GraphSpiralBall key={ball.id} vm={ball} />
          ))}
        </div>
      </section>
      <footer className="footer zuma-footer">
        <p>© 2026 MotionPath Graph Spiral Demo</p>
      </footer>
    </div>
  );
}
