...
function makeGraphBall(id) {
  ...
  const path = engine.createTrackInstance("ball-path", {
    id: pathConfig.id,
    duration: BALL_TRAVEL_SECONDS,
  });
  const entrance = engine.createTrackInstance("ball-entrance", {
    id: entranceConfig.id,
    duration: 0.35,
  });
  const exit = engine.createTrackInstance("ball-exit", {
    id: exitConfig.id,
    duration: 0.35,
  });
  ...
  const publisher = new GraphPublisher({
    graph,
    tracks,
    publish: (trackId, patch) => published.set(trackId, patch),
  });
...
export function useGraphSpiralController({ isLoaded }) {
    ...
    () =>
      setBallVms(
        ballsRef.current.map((ball) => ({
          ...ball,
          renderTrack: ball.status === "exiting" ? ball.exit : ball.entrance,
        })),
      ),
    [],
  );
  ...
  const remove = useCallback(
    (ball) => {
      hostRef.current?.track.removeChild(ball.path.id);
      dispose(ball);
  ...
  const entrance = useCallback(
    (ball) => {
      gsap.to(ball.entrance, {
        progress: 1,
        duration: 0.35,
        ease: "none",
        onComplete: () => {
          if (ball.status !== "spawning") return;
          ball.status = "active";
  ...
  const spawn = useCallback(() => {
    ...
    hostRef.current.track.addChild(ball.path, {
    ...
    const unsubscribe = ball.path.subscribe((snapshot) => {
      if (snapshot.progress >= 1) {
        unsubscribe();
        if (ball.status === "active") exit(ball);
      }
    });
    return ball;
  }, [entrance, exit, sync]);
  useEffect(() => {
    ...
    const nextHost = engine.createMotionHost({
      id: `graph-spiral-parent-${Date.now()}`,
      ...MOTION_HOST_OPTIONS,
    });
    ...
    const spawner = new Spawner({
      ...
      waveSize: WAVE_SIZE,
      canSpawn: () => {
        const last = ballsRef.current.at(-1);
        return !last || last.path.progress() >= MIN_SPAWN_PROGRESS;
      },
      factory: spawn,
      onComplete: () => {
        if (nextHost.track.childCount === 0) {
          spawner.resetWave();
          nextHost.motion.seek(0);
          nextHost.motion.play();
          spawner.start();
        }
      },
    });
    ...
    return () => {
      ...
      for (const ball of ballsRef.current) {
        nextHost.track.removeChild(ball.path.id);
        dispose(ball);
      }
      ballsRef.current = [];
      engine.unmount(nextHost.motion);
