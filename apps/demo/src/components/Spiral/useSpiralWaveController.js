...
export function useSpiralWaveController({ isLoaded }) {
    ...
    (ballId) => {
      ...
      overlay
        ...
        .then(() => {
          ...
          if (!latest) return;
          host?.track.removeChild(latest.ballTrack.id);
          spawnerRef.current?.notifyRemoved(1);
          removeBallVm(ballId);
        })
        .catch(() => {});
    },
    [disposeOverlay, getBallVm, host, removeBallVm, updateBallVm],
  );
  ...
  const spawnBall = useCallback(() => {
    ...
    setBallVms([...ballVmsRef.current]);
    host.track.addChild(ballTrack, { stagger: SPAWN_INTERVAL_MS / 1000 });
    startEntrance(id);
    ...
    return vm;
  }, [getBallVm, host, startEntrance, startExit]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      for (const vm of ballVmsRef.current) {
        disposeOverlay(vm);
        host.track.removeChild(vm.ballTrack.id);
        engine.unmount(vm.ballTrack);
      }
      ballVmsRef.current = [];
      setBallVms([]);
    };
  }, [disposeOverlay, host, isLoaded, spawnBall]);
  return { ballVms };
}
