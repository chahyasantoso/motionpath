...
export function createGraphSpiralBallMotion({
  ballSize,
  ballTravelSeconds,
  idPrefix = "ball",
}) {
...
export function createGraphSpiralProject({
  spiralPathPoints,
  ballSize,
  ballTravelSeconds,
}) {
  ...
  motion.tracks.find(
    (track) => track.id === "ball-path",
  ).keyframes.path.points = spiralPathPoints;
  return {
    schemaVersion: 4,
...
export function graphSpiralEdgeKeys(motion) {
  return motion.tracks.flatMap((track) =>
    (track.observes ?? []).map(
      (edge) => `${edge.source}->${track.id}:${edge.role ?? "output"}`,
    ),
  );
}
