...
const ENEMY_TYPES = {
  crawler: { hp: 3, speed: 0.002, emoji: "👾" },
  scout: { hp: 2, speed: 0.0035, emoji: "🛸" },
  goliath: { hp: 10, speed: 0.001, emoji: "🤖" },
};
...
export function useTowerDefenseController(isLoaded, enemyElsRef, projElsRef) {
  ...
  const towers = useRef(TOWERS_CONFIG.map((t) => ({ ...t })));
  const nextEnemy = useRef(1);
  const nextProjectile = useRef(1);
  const remainingToSpawn = useRef(0);
  ...
  const destroyMotion = (instance) => {
    if (instance) engine.unmount(instance);
  };
  ...
  const reset = useCallback(() => {
    enemies.current.forEach((e) => {
      destroyMotion(e.instance);
      destroyMotion(e.deathInstance);
    });
    projectiles.current.forEach((p) => destroyMotion(p.instance));
    enemies.current = [];
    projectiles.current = [];
    towers.current = TOWERS_CONFIG.map((t) => ({ ...t }));
    nextEnemy.current = 1;
    nextProjectile.current = 1;
    remainingToSpawn.current = 0;
  ...
  const startWave = useCallback(() => {
    ...
    remainingToSpawn.current = WAVE_ENEMY_COUNT + waveRef.current * 2;
    spawnElapsed.current = 0;
  ...
  useEffect(() => {
    ...
    const unsubscribe = gsapTickerClock.subscribe((deltaMs) => {
      ...
      if (remainingToSpawn.current > 0 && spawnElapsed.current >= 80) {
        spawnElapsed.current = 0;
        remainingToSpawn.current -= 1;
        const lane = Math.random() > 0.5 ? 1 : 2;
        const types =
          waveRef.current > 2
            ? ["crawler", "scout", "goliath"]
            : ["crawler", "scout"];
        const type =
          ENEMY_TYPES[types[Math.floor(Math.random() * types.length)]];
        const instance = mountManual(`lane-${lane}-path`);
        const id = nextEnemy.current++;
        enemies.current.push({
          ...
          maxHp: type.hp,
          hp: type.hp,
          speed: type.speed * (1 + waveRef.current * 0.1),
          progress: 0,
          x: 0,
          y: lane === 1 ? 150 : 350,
          rotation: 0,
      ...
      for (const enemy of enemies.current) {
        if (enemy.dying) {
          ...
          renderTrack(
            ...
            enemy.deathProgress,
            el,
            { x: enemy.deathX, y: enemy.deathY },
          );
          if (enemy.deathProgress >= 1) {
            destroyMotion(enemy.deathInstance);
        ...
        const points = enemy.lane === 1 ? LANE_1_POINTS : LANE_2_POINTS;
        const i = Math.min(
          points.length - 2,
          Math.floor(enemy.progress * (points.length - 1)),
        );
        const t = enemy.progress * (points.length - 1) - i;
        const a = points[i];
        const b = points[i + 1];
        enemy.x = a.x + (b.x - a.x) * t;
        enemy.y = a.y + (b.y - a.y) * t;
        enemy.rotation = (Math.atan2(b.y - a.y, b.x - a.x) * 180) / Math.PI;
        const el = enemyElsRef.current.get(enemy.id);
        if (el)
          domRenderer(el, { x: enemy.x, y: enemy.y, rotation: enemy.rotation });
        nextEnemies.push(enemy);
      }
      enemies.current = nextEnemies;
      for (const tower of towers.current) {
        if (tower.cooldown > 0) {
          tower.cooldown -= deltaFrames;
          continue;
        }
        ...
        if (target) {
          projectiles.current.push({
            ...
            startY: tower.y,
            progress: 0,
            instance: mountManual("projectile-style"),
          });
      ...
      for (const projectile of projectiles.current) {
        projectile.progress += PROJECTILE_SPEED * deltaFrames;
        const target = enemies.current.find(
          (enemy) => enemy.id === projectile.targetId && !enemy.dying,
        );
        if (!target || projectile.progress >= 1) {
          destroyMotion(projectile.instance);
          if (target && projectile.progress >= 1) {
            target.hp -= 1;
            if (target.hp <= 0) {
              target.dying = true;
              ...
              target.deathInstance = mountManual("enemy-death");
            }
          }
          continue;
        }
        const el = projElsRef.current.get(projectile.id);
        const x =
          projectile.startX +
          (target.x - projectile.startX) * projectile.progress;
        const y =
          projectile.startY +
          (target.y - projectile.startY) * projectile.progress;
        renderTrack(
          projectile.instance,
          "projectile-track",
          projectile.progress,
      ...
      setUiEnemies([...enemies.current]);
      setUiProjectiles([...projectiles.current]);
      if (
        remainingToSpawn.current === 0 &&
        enemies.current.length === 0 &&
        projectiles.current.length === 0
      )
        setGameState(waveRef.current >= 5 ? "won" : "idle");
    });
    return () => {
      unsubscribe();
      reset();
