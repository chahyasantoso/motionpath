...
function buildRig() { const tracks = new Map(walkerScene.tracks.map((config) => [config.id, createTrack(config)])); for (const config of walkerScene.tracks) for (const edge of config.observes || []) tracks.get(config.id).setObserved(tracks.get(edge.source), (patch) => ({ [edge.target]: patch || {} }), { role: edge.role }); return tracks; }
...
describe("fk walker rig", () => {
  it("authors a position on the pelvis only", () => { for (const track of walkerScene.tracks) { if (track.id === "pelvis") { expect(track.observes).toBeUndefined(); expect(Object.keys(track.keyframes).sort()).toEqual(["rotation", "x", "y"]); continue; } expect(Object.keys(track.keyframes).sort()).toEqual(["boneLength", "boneRotation"]); expect(track.observes).toEqual([{ source: expect.any(String), role: "input", target: "parentWorld" }]); } });
  it("keeps every joint at its authored bone length through the gait", () => { const tracks = buildRig(); for (const p of SAMPLES) { seek(tracks, p); expect(span(at(tracks, "pelvis"), at(tracks, "chest"))).toBeCloseTo(RIG.torso, 3); for (const side of ["near", "far"]) { expect(span(at(tracks, "pelvis"), at(tracks, `leg-${side}-shin`))).toBeCloseTo(RIG.thigh, 3); expect(span(at(tracks, `leg-${side}-shin`), at(tracks, `leg-${side}-foot`))).toBeCloseTo(RIG.shin, 3); expect(span(at(tracks, "chest"), at(tracks, `arm-${side}-fore`))).toBeCloseTo(RIG.upperArm, 3); } } });
