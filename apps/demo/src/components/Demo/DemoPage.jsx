...
} from "@motionpath/core/math/projection3d.js";
import {
  HELIX_CONFIG,
  demoProject,
...
const MOCK_CARDS = [
  {
    id: 1,
    badge: "01 / IMAGINATION",
    title: "Fluid Motion Engine",
    desc: "Harnessing the power of GSAP Pub/Sub for sub-millisecond DOM updates.",
  },
  {
    id: 2,
    badge: "02 / ARCHITECTURE",
    title: "Zero Re-renders",
    desc: "No React component updates during animation cycles for maximum 60FPS performance.",
  },
  {
    id: 3,
    badge: "03 / CREATIVE",
    title: "Bezier Interpolation",
    desc: "Custom paths calculated dynamically with tangent-aligned rotations.",
  },
  {
    id: 4,
    badge: "04 / INTERACTIVE",
    title: "Interactive Cards",
    desc: "Cards respond to mouse hover with smooth glassmorphic highlighting.",
  },
  {
    id: 5,
    badge: "05 / DESIGN",
    title: "Vibrant Aesthetics",
    desc: "Deep cosmic palettes, glassmorphism, and smooth typography curves.",
  },
  {
    id: 6,
    badge: "06 / PERFORMANCE",
    title: "Scrubbed Timelines",
    desc: "Perfect synchronization between page scroll and complex motion paths.",
  },
  {
    id: 7,
    badge: "07 / AWWWARDS",
    title: "Storytelling Layouts",
    desc: "Create immersive cinematic scrollytelling experiences that engage users.",
  },
  {
    id: 8,
    badge: "08 / ANTIGRAVITY",
...
function Rocket({ instance, offset = 0 }) {
  ...
  const transform = useCallback(
    (rawData, composeFn) => {
      const progress = Math.max(
        0,
        Math.min(1, (rawData.pathProgress ?? 0) + offset),
      );
  ...
  return (
    <div ref={ref} className="element rocket">
      🚀
    </div>
  );
}
function Cloud({ instance }) {
  ...
  return (
    <div ref={ref} className="element cloud">
      ☁️
    </div>
  );
}
function useChildTrack(parentTrack, trackId, keyframes, stagger) {
  ...
  useEffect(() => {
    ...
    setTrack(childTrack);
    return () => parentTrack.removeChild(trackId);
  }, [parentTrack, trackId]);
  return track;
}
function CarouselCard({ parentTrack, cardData, trackCfg, stagger, onRemove }) {
  ...
  const transform = useCallback(
    (rawData, composeFn) => {
      if (exitTrack) return composeFn(rawData);
      ...
      const targetTilt = Math.max(
        -20,
        Math.min(20, (composed.rotation ?? 0) * 0.35),
      );
      return {
        ...composed,
        display: "flex",
  ...
  useMotionSubscribers(
    activeTrack ? [{ track: activeTrack, transformFn: transform }] : [],
    ref,
  );
  const handleClick = useCallback(() => {
    if (exitTrack || !cardTrack) return;
  ...
  return (
    <div
      ref={ref}
      className="element carousel-card"
      onClick={handleClick}
...
function HelixCard({ parentTrack, cardData, trackCfg, stagger, trackId }) {
  ...
  const transform = useCallback((rawData, composeFn) => {
    ...
    const depthFactor =
      (point3D.z + HELIX_CONFIG.radius) / (2 * HELIX_CONFIG.radius);
  ...
  return (
    <section ref={refs.trigger} className="scroll-scene">
      <div className="scene-label">
        ...
        <p>
          A rocket following a 2D bezier path curve. The clouds move linearly on
          their own independent track.
        </p>
      </div>
      <div ref={refs.pin} className="stage">
        ...
        <Rocket instance={instance} offset={0} />
      </div>
    </section>
  );
}
function CarouselDemo({ isLoaded }) {
  const { refs, instance } = useScrollMotion(
    isLoaded ? dynamicCarouselScene : null,
  );
  ...
  return (
    <section ref={refs.trigger} className="carousel-scene">
      <div className="scene-label">
        ...
        <p>
          Dynamic mock cards flowing smoothly on a single Bezier S-curve track
          with engine-level stagger.
        </p>
      </div>
      <div ref={refs.pin} className="carousel-stage">
        ...
        <svg className="path-guide" width="100%" height="100%">
          <path
            d={buildMotionPath(
              dynamicCarouselScene.tracks[0].keyframes.path.points,
            )}
            fill="none"
            stroke="rgba(255,255,255,0.1)"
            strokeWidth="2"
        ...
        <button
          className="add-card-btn-floating"
...
function HelixDemo({ isLoaded }) {
  const { refs, instance } = useScrollMotion(
    isLoaded ? dynamicHelixScene : null,
  );
  ...
  return (
    <section ref={refs.trigger} className="helix-scene">
      <div className="scene-label">
        ...
        <p>
          Content cards flowing down a vertical spring, rotating 3D tangent to
  ...
  return (
    <div className="app">
      <header className="header">
        <h1>
          MotionPath <span className="accent">Hooks Demo</span>
        </h1>
