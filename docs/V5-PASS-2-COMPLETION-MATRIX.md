...
| Graph and patch values are immutable | Patch values deep-clone and freeze supported shapes; graph containers and direct records are frozen | Graph node/edge/diagnostic records are still shallow, and the supported value shape is undocumented | P2-01 | @chahyasantoso | Strict-mode mutation attempts |
