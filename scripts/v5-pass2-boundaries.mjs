...
for (const file of files) {
  ...
  if (GSAP_IMPORT_PATTERN.test(text) && !isApprovedGsapPath(rel)) {
    if (rel.startsWith(RENDERER_SURFACE)) addFinding("renderer-gsap-import", rel, "react hooks layer imports gsap directly; classify as adapter or migrate (F-14)", false);
    else addFinding("gsap-import", rel, isQuarantinedGsapPath(rel) ? "quarantined: known test/fixture import awaiting fake-port migration" : "unapproved direct vendor import outside adapters/", !isQuarantinedGsapPath(rel));
  }
  if (rel.endsWith("/Track.js")) {
    const observation = matched(TRACK_OBSERVATION_SYMBOLS, executableText(text));
    if (observation.length) addFinding("track-observation", rel, "Track still exposes observation ownership, state or mutation (P2-03)", strict, observation);
